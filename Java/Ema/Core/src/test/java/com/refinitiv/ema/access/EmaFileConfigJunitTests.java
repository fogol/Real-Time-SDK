...
public class EmaFileConfigJunitTests extends TestCase  
{
	...
	{
		String[] classpathEntries = (ManagementFactory.getRuntimeMXBean().getClassPath()).split(";");
		String choosenEntry = null;
		...
		if (choosenEntry == null) {
			TestUtilities.printTestHead("testLoadCfgFromFileInClasspath","Test case is skipped");
			return;
		}
...
//		According to new demand, if config file is specified, it should be loaded from filesystem.
//		Only default config file should be loaded from classpath

		// Prepare default config file for loading from classpath directory
		try {
			// Project environment
		...
		// Default config file should be removed from classpath directory.
		// otherwise it crash other tests
		try {
			if (Files.isRegularFile(Paths.get(choosenEntry + File.separator + "EmaConfig.xml"))) {
				Files.deleteIfExists(Paths.get(choosenEntry + File.separator + "EmaConfig.xml"));
			}
		} catch (IOException e) {
			System.out.println(e.getMessage());
		}
