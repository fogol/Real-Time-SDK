...
public class ArrayTests
{
	...
	void arrayEncodeDouble( Buffer buf, boolean fixedSize, double[] values, int numValues )
	{
		...
		com.refinitiv.eta.codec.Double value = CodecFactory.createDouble();
	...
	void arrayEncodeFloat( Buffer buf, boolean fixedSize, float[] values, int numValues )
	{
		...
		com.refinitiv.eta.codec.Float value = CodecFactory.createFloat();
	...
	void arrayEncodeInt(Buffer buf, boolean fixedSize, long values[], int numValues )
	{
		...
		com.refinitiv.eta.codec.Int value = CodecFactory.createInt();
	...
	void arrayEncodeUInt( Buffer buf, boolean fixedSize, long[] values, int numValues )
	{
		...
		array.itemLength(fixedSize ? 4 : 0); 
		array.primitiveType(DataTypes.UINT);
		com.refinitiv.eta.codec.UInt value = CodecFactory.createUInt();
	...
	void testArrayQos_Decode()
	{
		...
		try {
			...
			qos.timeliness(com.refinitiv.eta.codec.QosTimeliness.REALTIME);
			qos.rate(com.refinitiv.eta.codec.QosRates.TIME_CONFLATED);
			qos.dynamic(true);
			qos.rateInfo(9);
			qos.timeInfo( 0);
			assertEquals(arrayEntry.encode(iter,qos), CodecReturnCodes.SUCCESS);
			
			qos.timeliness(com.refinitiv.eta.codec.QosTimeliness.DELAYED);
			qos.rate(com.refinitiv.eta.codec.QosRates.JIT_CONFLATED);
			qos.dynamic(true);
			qos.rateInfo(0);
			qos.timeInfo( 15);
			assertEquals(arrayEntry.encode(iter,qos), CodecReturnCodes.SUCCESS);
	...
	void testArrayState_Decode()
	{
		...
		try {
			...
			state.streamState( com.refinitiv.eta.codec.StreamStates.CLOSED_RECOVER);
			state.dataState(com.refinitiv.eta.codec.DataStates.SUSPECT);
			state.code(com.refinitiv.eta.codec.StateCodes.TIMEOUT);
			state.text().data("Suspect Data");
			assertEquals(arrayEntry.encode(iter,state), CodecReturnCodes.SUCCESS);

			state.streamState( com.refinitiv.eta.codec.StreamStates.CLOSED);
			state.dataState(com.refinitiv.eta.codec.DataStates.SUSPECT);
			state.code(com.refinitiv.eta.codec.StateCodes.USAGE_ERROR);
	...
	void testArrayEnum_Decode( boolean fixedSize )
	{
		...
		try {
			...
			assertEquals(array.encodeInit(iter), CodecReturnCodes.SUCCESS);
		   com.refinitiv.eta.codec.Enum testEnum = CodecFactory.createEnum();
			testEnum.value(29);
