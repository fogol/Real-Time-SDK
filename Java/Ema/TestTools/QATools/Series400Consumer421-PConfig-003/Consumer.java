...
public class Consumer 
{
	static Map createProgramaticConfig()
	{
		...
		innerElementList.add(EmaFactory.createElementEntry().ascii( "InterfaceName", "consumerInterface"));
		//END APIQA
		...
		innerElementList.add(EmaFactory.createElementEntry().ascii( "Host", "PHost"));
		innerElementList.add(EmaFactory.createElementEntry().ascii("Port", "PPort"));
...
innerMap.add(EmaFactory.createMapEntry().keyAscii( "Channel_1", MapEntry.MapAction.ADD, innerElementList));
	...
	public static void main(String[] args)
	{
		...
		try
		{
	        ...
	        //APIQA	
			consumer  = EmaFactory.createOmmConsumer(config.config(createProgramaticConfig()).consumerName("Consumer_3").host("localhost:14002"));
