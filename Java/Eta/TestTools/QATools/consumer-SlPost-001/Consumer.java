 ...
 * on-stream).
 * <li>-slpost symbolListName. Application will attempt to post messages for
...
public class Consumer implements ResponseCallback, HttpCallback
{
    ...
    private final DirectoryHandler srcDirHandler;
    private final DictionaryHandler dictionaryHandler;
    private final MarketPriceHandler marketPriceHandler;
    private final MarketByOrderHandler marketByOrderHandler;
    private final MarketByPriceHandler marketByPriceHandler;
    private final PostHandler postHandler;
    private final SymbolListHandler symbolListHandler;
    private final YieldCurveHandler yieldCurveHandler;

    private boolean shouldOffStreamPost = false;
    private boolean shouldOnStreamPost = false;
    private boolean shouldSymbolListPost = false;
    private String symbolListName;
    private final Buffer postItemName;
    private final StreamIdWatchList itemWatchList;
    ...
    private final DecodeIterator dIter = CodecFactory.createDecodeIterator();
    private final Msg responseMsg = CodecFactory.createMsg();
    ...
    private final ConnectOptions _tunnelingConnectOpts;
    ...
    public void run()
    {
        ...
        while (true)
        {
            ...
            if (channelInfo.securityProtocol() != null && !channelInfo.securityProtocol().isEmpty())
            {
                System.out.println( "  Security Protocol: " + channelInfo.securityProtocol() + "\n");
            }
            else
            	System.out.print("\n");
            System.out.print("  Connected component version: ");

            int count = channelInfo.componentInfo() == null ? 0 : channelInfo.componentInfo().size();
            if (count == 0)
                System.out.print("(No component info)\n");
            else
            {
    ...
    private void handlePosting()
    {
        ...
        if (postHandler.enableOnstreamPost())
        {
            ...
            doPosting(postStreamId);
        }

        if (postHandler.enableSymbolListPost())
        {
            postHandler.postItemName().data(symbolListName);
            doPosting(loginHandler.refreshInfo().streamId());
        }
    ...
    private void doPosting(int postStreamId) {
        ...
        if (ret < CodecReturnCodes.SUCCESS)
            System.out.println("Error posting: " + error.text());
    }
    ...
    public void init(String[] args)
    {
        ...
        try
        {
            runtime = System.currentTimeMillis() + CommandLine.intValue("runtime") * 1000L;
        }
        ...
        symbolListName = CommandLine.value("slpost");
        shouldSymbolListPost = symbolListName != null && !symbolListName.isEmpty() && !symbolListName.equals("true");
        postHandler.enableSymbolListPost(shouldSymbolListPost);
    ...
    private void sendRequests(ChannelSession chnl)
    {
        ...
        {
            if (!srcDirHandler.serviceInfo().info().dictionariesProvidedList().isEmpty())
            {
    ...
    {
        if (!(shouldOnStreamPost || shouldOffStreamPost || shouldSymbolListPost) || postInit)
            return;
                ...
                loginHandler.refreshInfo().features().supportOMMPost() == 0)
        {
            ...
            postHandler.enableSymbolListPost(false);
    ...
    private void closeDictAndItemStreams()
    {
        ...
        if (shouldOffStreamPost)
        {
            ...
            postHandler.serviceId(srcDirHandler.serviceInfo().serviceId());
            postHandler.dictionary(dictionaryHandler.dictionary());
            postHandler.closeOffStreamPost(channelSession, DomainTypes.MARKET_PRICE, error);
        }
        ...
        if (shouldSymbolListPost)
        {
            ...
            postHandler.serviceId(srcDirHandler.serviceInfo().serviceId());
            postHandler.dictionary(dictionaryHandler.dictionary());
            postHandler.closeOffStreamPost(channelSession, DomainTypes.SYMBOL_LIST, error);
        }
    ...
    private static void addCommandLineArgs()
    {
        ...
        CommandLine.addOption("slpost", "Symbol List name. Specifies that the application should attempt to post messages for specified Symbol List name on the login stream (i.e., off-stream).");
