        ...
        ReactorServiceNameToIdCallback
{
    ...
    private void init(String[] args)
    {
        ...
        if(providerCmdLineParser.sendNack())
            sendNack = providerCmdLineParser.sendNack();

        if(providerCmdLineParser.rejectLogin())
            rejectLogin = providerCmdLineParser.rejectLogin();

        if(providerCmdLineParser.rejectTsLogin())
            rejectTsLogin = providerCmdLineParser.rejectTsLogin();
    ...
    public int rdmLoginMsgCallback(RDMLoginMsgEvent event)
    {
        ...
        if(rejectLogin)
        {
            if (loginHandler.sendRequestReject(reactorChannel, event.msg().streamId(), LoginRejectReason.NO_USER_NAME_IN_REQUEST, errorInfo) != CodecReturnCodes.SUCCESS)
                removeClientSessionForChannel(reactorChannel, errorInfo);
    ...
    public int listenerCallback(TunnelStreamRequestEvent event)
    {
        ...
        tunnelStreamHandler.processNewStream(event, sendNack, rejectTsLogin);
