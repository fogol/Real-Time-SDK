 ...
 * on-stream).
 * <li>-slpost symbolListName. The application will attempt to post messages for
 * specified Symbol List name on the login stream (i.e., off-stream).
		...
		ReactorJsonConversionEventCallback, ReactorServiceNameToIdCallback, ReactorOAuthCredentialEventCallback
{
	...
	{
		if (!(chnlInfo.shouldOnStreamPost || chnlInfo.shouldOffStreamPost || chnlInfo.shouldSymbolListPost))
			return;
			...
			chnlInfo.loginRefresh.features().supportOMMPost() == 0)
		{
			...
			chnlInfo.shouldSymbolListPost = false;
			chnlInfo.postHandler.enableOnstreamPost(false);
			chnlInfo.postHandler.enableSymbolListPost(false);
	...
	private void handlePosting()
	{
		for (ChannelInfo chnlInfo : chnlInfoList)
		{
			...
			{
				ItemInfo postingItem = getPostingItem(DomainTypes.MARKET_PRICE);
				if (postingItem == null)
				{
				...
				doPosting(chnlInfo, postingItem.streamId(), postingItem.name());
			}

			if (chnlInfo.postHandler.enableSymbolListPost())
			{
				doPosting(chnlInfo, chnlInfo.loginRefresh.streamId(), chnlInfo.symbolListName);
			}

			if (chnlInfo.postHandler.enableOffstreamPost())
			{
				doPosting(chnlInfo, chnlInfo.loginRefresh.streamId(), "OFFPOST");
			}
		}
	}

	private void doPosting(ChannelInfo chnlInfo, int streamId, String itemName) {
		chnlInfo.postHandler.streamId(streamId);
		chnlInfo.postHandler.postItemName().data(itemName);
		...
		if (ret < CodecReturnCodes.SUCCESS)
			System.out.println("Error posting: " + errorInfo.error().text());
	}

	private ItemInfo getPostingItem(int domain) {
		ItemInfo postingItem = null;

		for (int i = 0; i < watchlistConsumerConfig.itemCount(); i++) {
			if (watchlistConsumerConfig.itemList().get(i).domain() == domain) {
	...
	private void initChannelInfo(ChannelInfo chnlInfo)
	{
		...
		chnlInfo.shouldSymbolListPost = watchlistConsumerConfig.enableSymbolListPost();
		chnlInfo.symbolListName = watchlistConsumerConfig.symbolListName();

		if (chnlInfo.shouldOnStreamPost)
		{
			if (!isItemFoundInItemList(chnlInfo, DomainTypes.MARKET_PRICE))
			{
		...
		chnlInfo.postHandler.enableSymbolListPost(chnlInfo.shouldSymbolListPost);
	...
	private static boolean isItemFoundInItemList(ChannelInfo chnlInfo, int domain)
	{
	...
	private void closeItemStreams(ChannelInfo chnlInfo)
	{
		...
		if (chnlInfo.shouldOffStreamPost)
		{
			...
			chnlInfo.postHandler.serviceId(chnlInfo.serviceInfo.serviceId());
			chnlInfo.postHandler.dictionary(chnlInfo.dictionary);
			chnlInfo.postHandler.closeOffStreamPost(chnlInfo.reactorChannel, DomainTypes.MARKET_PRICE, errorInfo);
		}

		if (chnlInfo.shouldSymbolListPost)
		{
			chnlInfo.postHandler.streamId(chnlInfo.loginRefresh.streamId());
			chnlInfo.postHandler.postItemName().data(chnlInfo.symbolListName);
			chnlInfo.postHandler.serviceId(chnlInfo.serviceInfo.serviceId());
			chnlInfo.postHandler.dictionary(chnlInfo.dictionary);
			chnlInfo.postHandler.closeOffStreamPost(chnlInfo.reactorChannel, DomainTypes.SYMBOL_LIST, errorInfo);
		}
