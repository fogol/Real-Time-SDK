		...
		ReactorJsonConversionEventCallback, ReactorServiceNameToIdCallback, ReactorOAuthCredentialEventCallback
{
	...
	// APIQA
	private ArrayList<Integer> postItemsIndex = new ArrayList<>();
	private int currentPostIndex = 0;
	private State postingItemState;
	...
	int requestItems(Reactor reactor, ReactorChannel channel)
	{
		...
		// APIQA
		int itemListIndex = 0;
		LinkedList<Integer> usedDomains = new LinkedList<>();
		for(; itemListIndex < watchlistConsumerConfig.itemList().size(); itemListIndex++)
		{
			...
			{
				//System.out.println ("batchListIndex is" + batchListIndex);
			...
			// APIQA: Set the userSpecObj to a known value
			submitOptions.requestMsgOptions().userSpecObj(itemRequest.streamId() + 1024);
			// APIQA:
			System.out.println("\n\n----- APIQA: Submit: Setting UserSpec object to " + (itemRequest.streamId() + 1024) + " for streamid " + itemRequest.streamId() + "\n\n");
			// END APIQA
	...
	/* Requests the desired items.  */
	int requestItemsAgain(Reactor reactor, ReactorChannel channel)
	{
		...
		for(; itemListIndex < watchlistConsumerConfig.itemList().size(); itemListIndex++)
		{
			...
			/*  APIQA:  Batch support - commenting out this line
			itemRequest.addItem(watchlistConsumerConfig.itemList().get(itemListIndex).name);
		 	*  END APIQA */
			...
			itemRequest.domainType(domainType);
			itemRequest.streamId(watchlistConsumerConfig.itemList().get(itemListIndex).streamId + 50);
			//  APIQA
			for (int batchListIndex = itemListIndex; batchListIndex < watchlistConsumerConfig.itemList().size(); ++batchListIndex)
			{
				if (watchlistConsumerConfig.itemList().get(batchListIndex).domain() == watchlistConsumerConfig.itemList().get(itemListIndex).domain() &&
						(!usedDomains.contains(domainType)) && (watchlistConsumerConfig.itemList().get(batchListIndex).name != "" ))
				{
					itemRequest.addItem(watchlistConsumerConfig.itemList().get(batchListIndex).name);
				}
			}
			...
			else if (itemRequest.itemNames().size() > 1)
			{
				...
				watchlistConsumerConfig.itemList().add(itemListIndex, batchItemInfo);
				System.out.println("********* APIQA, requestItemsAgain: batch request streamID is " + batchItemInfo.streamId() + "\n\n\n");
			...
			submitOptions.requestMsgOptions().userSpecObj(itemRequest.streamId() + 1024);
			// APIQA:
			System.out.println("\n\n------ APIQA: Submit AGAIN: Setting UserSpec object to " + (itemRequest.streamId() + 1024) + " for streamid " + itemRequest.streamId() + "\n\n");
	...
	public int reactorChannelEventCallback(ReactorChannelEvent event)
	{
		...
		switch(event.eventType())
		{
			...
			case ReactorChannelEventTypes.CHANNEL_OPENED:
			{
				...
				{
					// APIQA: Commenting out requestItems
	...
	public int defaultMsgCallback(ReactorMsgEvent event)
	{
		...
		switch (msg.msgClass())
		{
			case MsgClasses.REFRESH:
				...
				// APIQA:
				System.out.println("\n\n------APIQA: Received Item Refresh, UserSpec on event's steamInfo is [" + event.streamInfo().userSpecObject() + "] \n\n");
				break;
			...
			case MsgClasses.UPDATE:
				...
				// APIQA:
				System.out.println("\n\n------APIQA: Received Item Update, UserSpec on event's steamInfo is [" + event.streamInfo().userSpecObject() + "] \n\n");
				// END APIQA

				/* Decode data body according to its domain. */
				itemDecoder.decodeDataBody(event.reactorChannel(), updateMsg);
				//APIQA: Request 2nd batch request
				eventCounter++;
				if (eventCounter == 10) {
					requestItemsAgain(event.reactorChannel().reactor(),event.reactorChannel());
				}
				// END APIQA
				break;
			...
			case MsgClasses.STATUS:
				...
				// APIQA
				System.out.println("------APIQA: Received Item Status, UserSpec on event's steamInfo is [" + event.streamInfo().userSpecObject() + "]");
				// END APIQA
		...
		switch (msgType)
		{
			case REFRESH:
						...
						|| itemDecoder.fieldDictionaryLoadedFromFile && itemDecoder.enumTypeDictionaryLoadedFromFile )
				{
					requestItems(reactor, event.reactorChannel());
					//requestItemsAgain(event.reactorChannel().reactor(),event.reactorChannel());
					//requestItemsAgain(reactor,event.reactorChannel());
				}
				//  END APIQA
				break;
			case UPDATE:
