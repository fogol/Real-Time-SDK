                ...
                while (iter.hasNext())
                {
                    ...
                    else if (key.isReadable()) // read from reactor channel
                    {
	                    ...
	                    if (ret == ReactorReturnCodes.FAILURE)
	                    {
                            ...
                            TunnelStream tunnelStream = _tunnelStreamHandlerHashMap.containsKey(reactorChnl) ?
                                                            _tunnelStreamHandlerHashMap.get(reactorChnl)._tunnelStream : null;
                       
                            if( tunnelStream != null)
                            {
                            	if (tunnelStream.info(tunnelStreamInfo, errorInfo) != ReactorReturnCodes.SUCCESS)
                            	{
                                	System.out.println("rsslTunnelStreamGetInfo failed: " + errorInfo);
                            	}
                            	else
                            	{
