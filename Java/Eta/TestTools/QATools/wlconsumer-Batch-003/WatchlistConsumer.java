		...
		ReactorJsonConversionEventCallback, ReactorServiceNameToIdCallback, ReactorOAuthCredentialEventCallback
{
	...
	int requestItems(Reactor reactor, ReactorChannel channel)
	{
		...
		for(; itemListIndex < watchlistConsumerConfig.itemList().size(); itemListIndex++)
		{
			...
			/*  APIQA:  commenting out next line
			...
			for (int batchListIndex = itemListIndex; batchListIndex < watchlistConsumerConfig.itemList().size(); ++batchListIndex)
			{
						...
						(!usedDomains.contains(domainType)))
				{
					...
					//System.out.println ("---- APIQA:  addingItem " + watchlistConsumerConfig.itemList().get(batchListIndex).name);
				}
			}
	...
	int requestMPItem(Reactor reactor, ReactorChannel channel, String name, boolean isSnap)
	{
		int ret;
		...
		itemRequest.domainType(DomainTypes.MARKET_PRICE);
		itemRequest.streamId(watchlistConsumerConfig.itemList().get(0).streamId() + watchlistConsumerConfig.itemList().size() + 2);
		itemRequest.streamId(50);
		System.out.println("---- APIQA: Requesting TRI again on user streamid  " + itemRequest.streamId());
	...
	public int defaultMsgCallback(ReactorMsgEvent event)
	{
		...
		switch (msg.msgClass())
		{
			case MsgClasses.REFRESH:
				...
				// APIQA
				System.out.println("\n\n------APIQA: Got Item Refresh, UserSpec on event's steamInfo is [" + event.streamInfo().userSpecObject() + "] \n\n");
				// END APIQA
				break;
			...
			case MsgClasses.UPDATE:
				...
				// APIQA
				System.out.println("\n\n--- APIQA: Got Item Update, UserSpec on event's steamInfo is [" + event.streamInfo().userSpecObject() + "] \n\n");
				// END APIQA
			...
			case MsgClasses.STATUS:
				...
				// APIQA
				System.out.println("---APIQA: Got Item Status, UserSpec on event's steamInfo is [" + event.streamInfo().userSpecObject() + "]");
				// END APIQA
