		...
		ReactorJsonConversionEventCallback, ReactorServiceNameToIdCallback, ReactorOAuthCredentialEventCallback
{
	...
	public int defaultMsgCallback(ReactorMsgEvent event)
	{
		...
		switch (msg.msgClass())
		{
			...
			case MsgClasses.UPDATE:
				...
				// APIQA:  Increment update counter
				updatesReceived++;
				// APIQA: Manufacture a dictionary reissue:
				if ((( updatesReceived % 6 ) == 0 ) && ( updatesReceived <= 30 ))
				{
					submitOptions.clear();
					submitOptions.serviceName(chnlInfo.serviceInfo.info().serviceName().toString());
					...
					requestMsg.streamId(ENUM_DICTIONARY_STREAM_ID);
					...
					requestMsg.msgKey().name().data(ENUM_TABLE_DOWNLOAD_NAME);
					if ( updatesReceived  == 6 || updatesReceived == 24 )
					{
						requestMsg.msgKey().filter(Dictionary.VerbosityValues.INFO);
					}
					else
					{
					...
					if ((chnlInfo.reactorChannel.submit(requestMsg, submitOptions, errorInfo)) !=  CodecReturnCodes.SUCCESS) {
						System.out.println("\n-------- APIQA: attempted DICTIONARY reissue with VerbosityValues of " + requestMsg.msgKey().filter() + " failed. Error: " + errorInfo.error().text() + "\n\n");
					}
					else
					{
						System.out.println("\n-------- APIQA: DICTIONARY reissue with VerbosityValues of " + requestMsg.msgKey().filter() + " done.\n\n");
					}
				}
				// END APIQA
	...
	public int rdmDictionaryMsgCallback(RDMDictionaryMsgEvent event)
	{
		...
		switch (msgType)
		{
			case REFRESH:
				// APIQA
				System.out.println("\n--------- APIQA: DICTIONARY REFRESH received.\n\n");
				// END APIQA
