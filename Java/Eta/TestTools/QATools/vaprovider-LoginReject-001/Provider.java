        ...
        ReactorServiceNameToIdCallback
{
	...
	// API QA:
	itemHandler.nItemsToSend(providerCmdLineParser.nItemsToSend());
	// End API QA
            ...
            if (keySet == null)
            {
                ...
                for (ReactorChannel reactorChnl : reactorChannelList)
                {
                    if (reactorChnl != null && reactorChnl.state() == ReactorChannel.State.READY)
                    {
                        ...
                        {
			    // API QA There is added code to send failure after sending out a certain number of updates across all items
			    if ( ret == 9999 ) {
			        loginHandler.sendCloseStatus(reactorChnl, errorInfo); 
			    }
			    // End API QA
			    else 
			    {
