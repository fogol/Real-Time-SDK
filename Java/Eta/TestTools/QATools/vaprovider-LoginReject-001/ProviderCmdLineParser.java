...
class ProviderCmdLineParser implements CommandLineParser
{
	...
	private int nItemsToSend = -1; // API QA
		...
		try
		{
			...
			while (argsCount < args.length)
			{
				...
				// API QA
					...
					nItemsToSend = Integer.parseInt(args[++argsCount]);
					++argsCount;
				// End API QA
	...
	public void printUsage()
	{
		System.out.println("Usage: Provider or\nProvider [-p <port number>] [-i <interface name>] [-s <service name>] [-id <service ID>] [-runtime <seconds>]" +
						   ...
						   "\n -ni <value> where value represents the number of updates to send before sending Login reject)\n" +
