 ...
 * <li>-at2 Specifies a 2nd Authentication Token used when doing a reissue. If this is present, the login user name type will be Login.UserIdTypes.AUTHN_TOKEN.
		...
		ReactorJsonConversionEventCallback, ReactorServiceNameToIdCallback, ReactorOAuthCredentialEventCallback
{
    ...
    // APIQA: adding newItemRequested
    boolean newItemRequested = false;
		...
		while (true)
		{
			...
			if (!closeHandled)
			{
				...
				for (ChannelInfo chnlInfo : chnlInfoList)
				{
					...
					{
						/* APIQA: Use 2nd token in reissue */
						...
						/* END APIQA: Use 2nd token in reissue */
	...
	// APIQA: Doing item reissue on item stream
	int reissueItem(Reactor reactor, ReactorChannel channel, int userStreamIndex, boolean pauseFlag)
	{
		System.out.println("-----------------APIQA: RE-requesting TRI again on user streamid  " + watchlistConsumerConfig.itemList().get(userStreamIndex).streamId);
		...
		if (pauseFlag)
		{
			System.out.println("-----------------APIQA: Setting PAUSE and NO_REFRESH flags on item reissue for user streamid " + watchlistConsumerConfig.itemList().get(userStreamIndex).streamId);
			itemRequest.applyPause();
			itemRequest.applyNoRefresh();
		}

		itemRequest.addItem(watchlistConsumerConfig.itemList().get(userStreamIndex).name);
		int domainType = watchlistConsumerConfig.itemList().get(userStreamIndex).domain();
		itemRequest.domainType(domainType);
		itemRequest.streamId(watchlistConsumerConfig.itemList().get(userStreamIndex).streamId);
	...
	// END APIQA: Reissuing item request
	...
	public int defaultMsgCallback(ReactorMsgEvent event)
	{
		...
		switch (msg.msgClass())
		{
			case MsgClasses.REFRESH:
				...
				// APIQA: Do reissue
				// Assumption: user is requesting TRI TRI IBM in that order
				eventReceived++;
				if (eventReceived == 5)
					reissueItem(reactor, event.reactorChannel(), 0, true);
				if (eventReceived == 10)
					reissueItem(reactor, event.reactorChannel(), 1, true);
				if (eventReceived == 25)
					reissueItem(reactor, event.reactorChannel(), 0, false);
				if (eventReceived == 30)
					reissueItem(reactor, event.reactorChannel(), 1, false);
				// END APIQA
				break;
