...
RsslRet sendSymbolListPostMsg(RsslReactor* pReactor, RsslReactorChannel* chnl);
...
RsslRet handlePosts(RsslReactor* pReactor, ChannelCommand* pCommand)
{
	...
	if (currentTime >= pCommand->nextPostTime)
	{
		...
		if (pCommand->shouldSymbolListPost == RSSL_TRUE)
		{
			if (sendSymbolListPostMsg(pReactor, pCommand->reactorChannel) != RSSL_RET_SUCCESS)
				return RSSL_RET_FAILURE;
		}
 ...
 * contents.
 */
static RsslRet encodePostWithMsg(RsslReactorChannel* chnl, RsslBuffer* msgBuf, int domainType, RsslInt32 streamId, RsslBuffer* itemName)
{
	RsslRet ret = 0;
	...
	itemInfo.domainType = domainType;
	...
	else if (domainType == RSSL_DMT_SYMBOL_LIST)
	{
		...
		{
			ret = encodeSymbolListResponse(chnl, &itemInfo, &payloadMsgBuf, isSolicited, streamId, RSSL_FALSE, (RsslUInt16)pCommand->serviceId, dictionary, itemName->data, SYMBOL_LIST_REFRESH);
			if (ret != RSSL_RET_SUCCESS)
			{
		...
		else
		{
			...
			ret = encodeSymbolListResponse(chnl, &itemInfo, &payloadMsgBuf, RSSL_TRUE, streamId, RSSL_FALSE, (RsslUInt16)pCommand->serviceId, dictionary, "IBM", responseType);
			if (ret != RSSL_RET_SUCCESS)
			{
				printf("encodePostWithMsg: encodeSymbolListResponse() failed\n");
				return RSSL_RET_FAILURE;
			...
			if (slUpdateAdd == RSSL_TRUE) slUpdateAdd = RSSL_FALSE; else slUpdateAdd = RSSL_TRUE;
		}
	}
	else
	{
		printf("encodePostWithMsg: unsupported domain type: %d\n", domainType);
		return RSSL_RET_FAILURE;
	}
...
RsslRet sendOnstreamPostMsg(RsslReactor* pReactor, RsslReactorChannel* chnl, RsslBool postWithMsg)
{
	...
	if (msgBuf != NULL)
	{
		if (pCommand->postWithMsg == RSSL_TRUE)
		{
			if (encodePostWithMsg(chnl, msgBuf, RSSL_DMT_MARKET_PRICE, pRequest->streamId, &pRequest->itemName) != RSSL_RET_SUCCESS)
			{
...
RsslRet sendOffStreamPostMsg(RsslReactor* pReactor, RsslReactorChannel* chnl)
{
	...
	if (msgBuf != NULL)
	{
		...
		if (encodePostWithMsg(chnl, msgBuf, RSSL_DMT_MARKET_PRICE, streamId, &nameBuf) != RSSL_RET_SUCCESS)
		{
			rsslReactorReleaseBuffer(chnl, msgBuf, &rsslErrorInfo);
			printf("\nencodePostWithMsg() failed\n");
			return RSSL_RET_FAILURE;
...
RsslRet sendSymbolListPostMsg(RsslReactor* pReactor, RsslReactorChannel* chnl)
{
	...
	/* get the login stream id to send symbol list post */
	streamId = pCommand->loginRefresh.rdmMsgBase.streamId;
	...
	if (msgBuf != NULL)
	{
		...
		if (encodePostWithMsg(chnl, msgBuf, RSSL_DMT_SYMBOL_LIST, streamId, &nameBuf) != RSSL_RET_SUCCESS)
		{
			rsslReactorReleaseBuffer(chnl, msgBuf, &rsslErrorInfo);
			printf("\nencodePostWithMsg() failed\n");
			return RSSL_RET_FAILURE;
