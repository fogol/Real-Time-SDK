	...
	do
	{
		...
		if (pConsumerChannel && !runTimeExpired && !loginChannelClosed)
		{
			...
			if (currentTime >= nextPostTime)
			{
				...
				if (watchlistConsumerConfig.slPost && isConsumerChannelUp && serviceInfo.isServiceFound && serviceInfo.isServiceUp)
					sendSymbolListPostMsg(pReactor, pConsumerChannel);
