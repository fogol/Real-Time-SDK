...
static RsslUInt32 onStreamPostId, offStreamPostId, slPostId;
static RsslUInt32 onStreamSeqNum, offStreamSeqNum, slSeqNum;
...
static RsslBool slPostRefresh;
static RsslBool slUpdateAdd;
...
void postHandlerInit()
{
	...
	slPostRefresh = RSSL_TRUE;
	slUpdateAdd = RSSL_TRUE;
		...
		RsslBool postWithMsg)
{
	...
	if (postWithMsg)
		return encodePostWithMsg(pReactor, pReactorChannel, RSSL_DMT_MARKET_PRICE, streamId, onStreamPostId, onStreamSeqNum, &watchlistConsumerConfig.itemList[i].name, &onStreamItemInfo);
	else
		...
		RsslBool postWithMsg)
{
	...
	if (postWithMsg)
		return encodePostWithMsg(pReactor, pReactorChannel, RSSL_DMT_MARKET_PRICE, LOGIN_STREAM_ID, offStreamPostId, offStreamSeqNum, &nameBuf, &offStreamItemInfo);
	else
...
/* Sends a post on the login stream ("off-stream"). */
RsslRet sendSymbolListPostMsg(RsslReactor* pReactor, RsslReactorChannel* pReactorChannel)
{
	RsslBuffer nameBuf;
	...
	nameBuf.data = (char*)"_SYMBOL_LIST_POST";
	nameBuf.length = (RsslUInt32)strlen("_SYMBOL_LIST_POST");

	return encodePostWithMsg(pReactor, pReactorChannel, RSSL_DMT_SYMBOL_LIST, LOGIN_STREAM_ID, slPostId, slSeqNum, &nameBuf, NULL);
}

/* Encodes a post with a nested message as its payload. */
static RsslRet encodePostWithMsg(RsslReactor *pReactor, RsslReactorChannel *pReactorChannel, RsslUInt8 domainType,
		...
		RsslBuffer *itemName, PostItemInfo* itemInfo)
{
	...
	postMsg.msgBase.domainType = domainType;
	...
	else if (domainType == RSSL_DMT_SYMBOL_LIST)
	{
		RsslItemInfo slItemInfo;
		if (slPostRefresh == RSSL_TRUE)
		{
			ret = encodeSymbolListResponse(pReactorChannel, &slItemInfo, &payloadMsgBuf, RSSL_TRUE, streamId, RSSL_FALSE, 256, NULL, itemName->data, SYMBOL_LIST_REFRESH);
			slPostRefresh = RSSL_FALSE;
		}
		else
		{
			RsslUInt8 responseType;
			if (slUpdateAdd == RSSL_TRUE) responseType = SYMBOL_LIST_UPDATE_ADD;
			else responseType = SYMBOL_LIST_UPDATE_DELETE;

			ret = encodeSymbolListResponse(pReactorChannel, &slItemInfo, &payloadMsgBuf, RSSL_TRUE, streamId, RSSL_FALSE, 256, NULL, "IBM", responseType);

			if (slUpdateAdd == RSSL_TRUE) slUpdateAdd = RSSL_FALSE; else slUpdateAdd = RSSL_FALSE;
		}
		
		if (ret != RSSL_RET_SUCCESS)
		{
			printf("encodePostWithMsg: encodeSymbolListResponse() failed\n");
			return RSSL_RET_FAILURE;
		}
	}	
	else
	{
		printf("encodePostWithMsg: unsupported domain type\n");
		return RSSL_RET_FAILURE;
	}
