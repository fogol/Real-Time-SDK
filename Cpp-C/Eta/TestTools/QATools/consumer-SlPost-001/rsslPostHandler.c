...
static RsslBool	shouldOffstreamPost = RSSL_FALSE, shouldOnstreamPost = RSSL_FALSE, shouldSymbolListPost = RSSL_FALSE;
...
RsslRet sendSymbolListPostMsg(RsslChannel* chnl);
...
void enableSymbolListPost()
{
	shouldSymbolListPost = RSSL_TRUE;
}

/* disables Symbol List stream posting mode */
void disableSymbolListPost()
{
	shouldSymbolListPost = RSSL_FALSE;
}
...
RsslRet handlePosts(RsslChannel* chnl, RsslBool connectionRecovery)
{
	...
	if (currentTime >= nextPostTime)
	{
		...
		{
			if (sendSymbolListPostMsg(chnl) != RSSL_RET_SUCCESS)
				return RSSL_RET_FAILURE;
		}
 ...
 * contents.
 */
static RsslRet encodePostWithMsg(RsslChannel* chnl, RsslBuffer* msgBuf, int domainType, RsslInt32 streamId, RsslBuffer *itemName)
{
	RsslRet ret = 0;
	...
	else if (domainType == RSSL_DMT_SYMBOL_LIST)
	{
		...
		{
			ret = encodeSymbolListResponse(chnl, &itemInfo, &payloadMsgBuf, isSolicited, streamId, RSSL_FALSE, serviceId, dictionary, itemName->data, SYMBOL_LIST_REFRESH);
			if (ret != RSSL_RET_SUCCESS)
			{
		...
		else
		{
			...
			ret = encodeSymbolListResponse(chnl, &itemInfo, &payloadMsgBuf, RSSL_TRUE, streamId, RSSL_FALSE, serviceId, NULL, "IBM", responseType);
			if (ret != RSSL_RET_SUCCESS)
			{
				printf("encodePostWithMsg: encodeSymbolListResponse() failed\n");
				return RSSL_RET_FAILURE;
