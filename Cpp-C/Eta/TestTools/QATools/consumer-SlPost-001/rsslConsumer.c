...
static RsslBool onPostEnabled = RSSL_FALSE, offPostEnabled = RSSL_FALSE, slPostEnabled = RSSL_FALSE;
...
void printUsageAndExit(char *appName)
{
	...
	printf(" -offpost specifies that the application should attempt to send Symbol List post messages on the login stream (i.e., off-stream)\n");
	printf(" -slpost specifies that the application should attempt to send post messages on the login stream (i.e., off-stream)\n");
...
int main(int argc, char **argv)
{
	...
	else if (argc > 1) /* all operating parameters entered by user */
	{
		...
		while(i < argc)
		{
			...
			else if (strcmp("-slpost", argv[i]) == 0)
			{
				i++;
				slPostEnabled = RSSL_TRUE;
				enableSymbolListPost();
			}
	...
	while(1)
	{
		...
		while(1)
		{
			...
			if (onPostEnabled || offPostEnabled || slPostEnabled)
			{
