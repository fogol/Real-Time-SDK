 ...
 *|           Copyright (C) 2016,2018-2022,2024-2025 LSEG. All rights reserved.
 *|-----------------------------------------------------------------------------
 */
...
bool EmaCppConsPerf::initConsPerfConfig(int argc, char *argv[])
{
	...
	while(iargs < argc)
	{
		...
		else if (strcmp("-enableMessageQueue", argv[iargs]) == 0)
		{
			++iargs;
			consPerfConfig.enableMessageQueue = true;
		}
		else if (strcmp("-cloneMsgBufSize", argv[iargs]) == 0)
		{
			++iargs;
			if (iargs == argc)
			{
			...
			consPerfConfig.cloneMsgBufSize = atoi(argv[iargs++]);
		}
		else if (strcmp("-refreshMsgPoolSize", argv[iargs]) == 0)
		{
			++iargs;
			if (iargs == argc)
			{
			...
			consPerfConfig.refreshMsgPoolSize = atoi(argv[iargs++]);
		}
		else if (strcmp("-updateMsgPoolSize", argv[iargs]) == 0)
		{
			++iargs;
			if (iargs == argc)
			{
			...
			consPerfConfig.updateMsgPoolSize = atoi(argv[iargs++]);
		}
		else if (strcmp("-statusMsgPoolSize", argv[iargs]) == 0)
		{
			++iargs;
			if (iargs == argc)
			{
			...
			consPerfConfig.statusMsgPoolSize = atoi(argv[iargs++]);
		}
		else
		{
...
void EmaCppConsPerf::printConsPerfConfig(FILE *file)
{
	...
	fprintf(file,
		...
		consPerfConfig.enableMessageQueue ? "Yes" : "No",
...
bool EmaCppConsPerf::inititailizeAndRun( int argc, char *argv[])
{
	...
	for( i = 0; i < consPerfConfig.threadCount; ++i)
	{
		...
		if (consPerfConfig.enableMessageQueue)
		{
			MessageQueueThread* messageQueueThread = new MessageQueueThread(consPerfConfig);
			messageQueueThread->setConsumerThread(pconsumerThread);
			pconsumerThread->setMessageQueueThread(messageQueueThread);
			messageQueueThreads.push_back(messageQueueThread);
		}
