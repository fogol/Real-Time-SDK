...
ConsumerThread::ConsumerThread( ConsPerfConfig& consPerfCfg) :
	...
	stopThread(false),
	statsFile( nullptr ),
	latencyLogFile( nullptr ),
	desiredServiceHandle(0),
	isDesiredServiceUp(false),
	itemsRequestedCount(0),
	refreshCompleteCount(0),
	latencyPostRandomArray( nullptr ),
	latencyGenericRandomArray( nullptr ),
	consThreadState(consPerfCfg),
	testPassed(true),
	pEmaOmmConsumer( nullptr ),
	_pThread( nullptr ),
	_pMessageQueueThread( nullptr )
{
}
...
void  ConsumerThread::clean()
{
	...
	if (latencyPostRandomArray != nullptr)
		delete latencyPostRandomArray;
	latencyPostRandomArray = nullptr;

	if (latencyGenericRandomArray != nullptr)
...
{
	if (_pMessageQueueThread)
		_pMessageQueueThread->start();

	_pThread = new std::thread(&ConsumerThread::run, this);
...
void ConsumerThread::setMessageQueueThread(MessageQueueThread* messageQueueThread)
{
	_pMessageQueueThread = messageQueueThread;
}
...
void ConsumerThread::dumpConsumerItemList()
{
	...
	for (UInt64 i = 0; i < itemRequestList.size(); ++i)
	{
...
void ConsumerThread::run()
{
	...
	try {
		while (running && !stopThread && !isDesiredServiceUp)
		{
			if( pConsPerfCfg->useUserDispatch )
...
bool ConsumerThread::sendItemRequestBurst(UInt32 itemBurstCount)
{
	for( UInt32 i = 0; i < itemBurstCount && !stopThread; i++)
	{
		...
		itemRequestList[itemsRequestedCount]->itemInfo.handle = pEmaOmmConsumer->registerClient(requestMsg,
			*(itemRequestList[itemsRequestedCount]->itemInfo.pAppClient),
			(void *)itemRequestList[itemsRequestedCount]);
...
bool ConsumerThread::sendPostBurst(UInt32 postItemBurstCount)
{
	...
	const ItemRequest* itemReq = nullptr;
	PostMsg postMsg;
...
bool MarketPriceClient::decodeMPUpdate(ConsumerThread* pConsumerThread, const refinitiv::ema::access::FieldList& fldList, UInt16 msgtype )
{
	Int64		intType;
	UInt64		uintType = 0;
	float		floatType;
...
void processMpcOnRefreshMsg(const RefreshMsg& refresh, ConsumerThread* pConsumerThread, void* closure)
{
	pConsumerThread->stats.refreshCount.countStatIncr();

	if( !MarketPriceClient::decodeMPUpdate(pConsumerThread, refresh.getPayload().getFieldList(),  DataType::RefreshMsgEnum) )
	{
	...
	if(!pConsumerThread->stats.imageRetrievalEndTime)
	{
		...
		{
			ItemRequest *pItemClosure = (ItemRequest*) closure;
...
void processMpcOnUpdateMsg(const UpdateMsg& update, ConsumerThread* pConsumerThread)
{
	pConsumerThread->stats.imageRetrievalEndTime
		? pConsumerThread->stats.steadyStateUpdateCount.countStatIncr()
		: pConsumerThread->stats.startupUpdateCount.countStatIncr();

	if(!(pConsumerThread->stats.firstUpdateTime))
		pConsumerThread->stats.firstUpdateTime = perftool::common::GetTime::getTimeNano();

	if( !MarketPriceClient::decodeMPUpdate(pConsumerThread, update.getPayload().getFieldList(),  DataType::UpdateMsgEnum) )
	{
		pConsumerThread->stopThread = true;
		pConsumerThread->testPassed = false;
...
void processMpcOnStatusMsg(const StatusMsg& stMsg, ConsumerThread* pConsumerThread)
{
...
void processMpcOnGenericMsg(const GenericMsg& genericMsg, ConsumerThread* pConsumerThread)
{
	...
	if ( !MarketPriceClient::decodeMPUpdate(pConsumerThread, genericMsg.getPayload().getFieldList(), DataType::GenericMsgEnum) )
	{
		pConsumerThread->stopThread = true;
		pConsumerThread->testPassed = false;
...
{
	if (pConsumerThread->_pMessageQueueThread)
		pConsumerThread->_pMessageQueueThread->enqueue(refresh, pConsumerThread, msgEvent.getClosure());
	else
		processMpcOnRefreshMsg(refresh, pConsumerThread, msgEvent.getClosure());
}
...
{
	if (pConsumerThread->_pMessageQueueThread)
		pConsumerThread->_pMessageQueueThread->enqueue(update, pConsumerThread);
	else
		processMpcOnUpdateMsg(update, pConsumerThread);
}
...
{
	if (pConsumerThread->_pMessageQueueThread)
		pConsumerThread->_pMessageQueueThread->enqueue(stMsg, pConsumerThread);
	else
		processMpcOnStatusMsg(stMsg, pConsumerThread);
}

void MarketPriceClient::onGenericMsg( const GenericMsg& genericMsg, const OmmConsumerEvent& consumerEvent )
{
	if (pConsumerThread->_pMessageQueueThread)
		pConsumerThread->_pMessageQueueThread->enqueue(genericMsg, pConsumerThread);
	else
		processMpcOnGenericMsg(genericMsg, pConsumerThread);
}

void MarketPriceClient::onAckMsg( const AckMsg& ackMsg, const OmmConsumerEvent& )
{
	AppUtil::log(" Received AckMsg on Domain %u Processing not ready \n", (UInt32) ackMsg.getDomainType());
}


bool MarketByOrderClient::decodeMBOUpdate(ConsumerThread* pConsumerThread, const refinitiv::ema::access::Map& mboMap, UInt16 msgtype  )
{
	UInt64 timeTracker = 0;
	UInt64 postTimeTracker = 0;
	UInt64 genMsgTimeTracker = 0;
	...
	{
		if( !MarketByOrderClient::decodeFldList( mboMap.getSummaryData().getFieldList(), msgtype, timeTracker, postTimeTracker, genMsgTimeTracker, true ) )
		{
	...
	while( mboMap.forth() )
	{
		...
		if( me.getAction() != MapEntry::DeleteEnum )
		{
			...
			if( !MarketByOrderClient::decodeFldList( me.getFieldList(), msgtype, timeTracker, postTimeTracker, genMsgTimeTracker, false ) )
			{
...
void processMboOnRefreshMsg(const RefreshMsg& refresh, ConsumerThread* pConsumerThread, void* closure)
{
	pConsumerThread->stats.refreshCount.countStatIncr();

	if( !MarketByOrderClient::decodeMBOUpdate(pConsumerThread, refresh.getPayload().getMap(),  DataType::RefreshMsgEnum) )
	{
		pConsumerThread->stopThread = true;
		pConsumerThread->testPassed = false;
...
void processMboOnUpdateMsg( const UpdateMsg& update, ConsumerThread* pConsumerThread )
{
	...
	if( !MarketByOrderClient::decodeMBOUpdate(pConsumerThread, update.getPayload().getMap(),  DataType::UpdateMsgEnum) )
	{
		pConsumerThread->stopThread = true;
		pConsumerThread->testPassed = false;
...
void processMboOnGenericMsg( const GenericMsg& genericMsg, ConsumerThread* pConsumerThread )
{
	pConsumerThread->stats.genMsgRecvCount.countStatIncr();
	if ( !pConsumerThread->stats.firstGenMsgRecvTime )
		pConsumerThread->stats.firstGenMsgRecvTime = perftool::common::GetTime::getTimeNano();

	if ( !MarketByOrderClient::decodeMBOUpdate(pConsumerThread, genericMsg.getPayload().getMap(), DataType::GenericMsgEnum) )
	{
		pConsumerThread->stopThread = true;
		pConsumerThread->testPassed = false;
...
void MarketByOrderClient::onRefreshMsg( const refinitiv::ema::access::RefreshMsg& refresh, const refinitiv::ema::access::OmmConsumerEvent&  msgEvent)
{
	if (pConsumerThread->_pMessageQueueThread)
		...
		processMboOnRefreshMsg(refresh, pConsumerThread, msgEvent.getClosure());
}

void MarketByOrderClient::onUpdateMsg( const refinitiv::ema::access::UpdateMsg& update, const refinitiv::ema::access::OmmConsumerEvent& )
{
	if (pConsumerThread->_pMessageQueueThread)
		pConsumerThread->_pMessageQueueThread->enqueue(update, pConsumerThread);
	else
		processMboOnUpdateMsg(update, pConsumerThread);
}
...
void MarketByOrderClient::onGenericMsg( const GenericMsg& genericMsg, const OmmConsumerEvent& )
{
	if (pConsumerThread->_pMessageQueueThread)
		pConsumerThread->_pMessageQueueThread->enqueue(genericMsg, pConsumerThread);
	else
		processMboOnGenericMsg(genericMsg, pConsumerThread);
}

void MarketByOrderClient::onAckMsg( const AckMsg& ackMsg, const OmmConsumerEvent& )
{
	AppUtil::log(" Received AckMsg on Domain %u Processing not ready \n", (UInt32) ackMsg.getDomainType());
}
...
void DirectoryClient::onRefreshMsg( const refinitiv::ema::access::RefreshMsg& refreshMsg, const refinitiv::ema::access::OmmConsumerEvent&)
{
...
void DirectoryClient::onUpdateMsg( const refinitiv::ema::access::UpdateMsg& updMsg, const refinitiv::ema::access::OmmConsumerEvent&)
{
...
void DirectoryClient::onStatusMsg( const refinitiv::ema::access::StatusMsg& stMsg, const refinitiv::ema::access::OmmConsumerEvent&)
{
...
void populatePool(ConcurrentQueue<MsgType*>& pool, std::vector<MsgType>& allMsgs, UInt32 messageSize)
{
	while (! pool.full())
	{
...
MessageQueueThread::MessageQueueThread(const ConsPerfConfig& consPerfCfg) :
	_pConsThread(nullptr),
	_exit(false),
	_thread(nullptr),
	_messageQueue(consPerfCfg.refreshMsgPoolSize
		+ consPerfCfg.updateMsgPoolSize
		+ consPerfCfg.statusMsgPoolSize
	),
	_refreshMsgPool(consPerfCfg.refreshMsgPoolSize),
	_updateMsgPool(consPerfCfg.updateMsgPoolSize),
	_statusMsgPool(consPerfCfg.statusMsgPoolSize),
	_genericMsgPool(consPerfCfg.genericMsgPoolSize),
	_allRefreshMsgs(),
	_allUpdateMsgs(),
	_allStatusMsgs(),
	_allGenericMsgs()
{
	_allRefreshMsgs.reserve(consPerfCfg.refreshMsgPoolSize);
	_allUpdateMsgs.reserve(consPerfCfg.refreshMsgPoolSize);
	_allStatusMsgs.reserve(consPerfCfg.refreshMsgPoolSize);
	_allGenericMsgs.reserve(consPerfCfg.refreshMsgPoolSize);

	populatePool<RefreshMsg>(_refreshMsgPool, _allRefreshMsgs, consPerfCfg.cloneMsgBufSize);
	populatePool<UpdateMsg>(_updateMsgPool, _allUpdateMsgs, consPerfCfg.cloneMsgBufSize);
	populatePool<StatusMsg>(_statusMsgPool, _allStatusMsgs, consPerfCfg.cloneMsgBufSize);
	populatePool<GenericMsg>(_genericMsgPool, _allGenericMsgs, consPerfCfg.cloneMsgBufSize);
}
...
void MessageQueueThread::setConsumerThread(ConsumerThread* pConsThread)
{
	_pConsThread = pConsThread;
}

void MessageQueueThread::start()
{
	_exit = false;
	_thread = new std::thread(&MessageQueueThread::run, this);
}

void MessageQueueThread::stop()
{
	_exit = true;
	if (_thread != nullptr)
	{
...
void MessageQueueThread::enqueue(const RefreshMsg& refreshMsg, ConsumerThread* pConsumerThread, void* closure)
{
	if (_exit) return;

	RefreshMsg* msg = _refreshMsgPool.pop();
	*msg = refreshMsg;
	_messageQueue.push({ msg, pConsumerThread, closure });
}

void MessageQueueThread::enqueue(const UpdateMsg& updateMsg, ConsumerThread* pConsumerThread)
{
	if (_exit) return;

	UpdateMsg* msg = _updateMsgPool.pop();
	*msg = updateMsg;
	_messageQueue.push({ msg, pConsumerThread, nullptr });
}

void MessageQueueThread::enqueue(const StatusMsg& statusMsg, ConsumerThread* pConsumerThread)
{
	if (_exit) return;
...
void MessageQueueThread::enqueue(const GenericMsg& genericMsg, ConsumerThread* pConsumerThread)
{
	if (_exit) return;

	GenericMsg* msg = _genericMsgPool.pop();
	*msg = genericMsg;
	_messageQueue.push({ msg, pConsumerThread, nullptr });
}
...
void MessageQueueThread::run()
{
	...
	while (!_exit)
	{
		...
		{
			switch (queuedMessage.pMsg->getDataType())
			{
			case DataType::RefreshMsgEnum:
			{
				RefreshMsg* refreshMsg = static_cast<RefreshMsg*>(queuedMessage.pMsg);

				if (refreshMsg->getDomainType() == refinitiv::ema::rdm::MMT_MARKET_PRICE)
					processMpcOnRefreshMsg(*refreshMsg, queuedMessage.pConsumerThread, queuedMessage.pClosure);
				else
					processMboOnRefreshMsg(*refreshMsg, queuedMessage.pConsumerThread, queuedMessage.pClosure);
			...
			{
				UpdateMsg* updateMsg = static_cast<UpdateMsg*>(queuedMessage.pMsg);

				if (updateMsg->getDomainType() == refinitiv::ema::rdm::MMT_MARKET_PRICE)
					processMpcOnUpdateMsg(*updateMsg, queuedMessage.pConsumerThread);
				else
					processMboOnUpdateMsg(*updateMsg, queuedMessage.pConsumerThread);
			...
			{
				StatusMsg* statusMsg = static_cast<StatusMsg*>(queuedMessage.pMsg);

				if (statusMsg->getDomainType() == refinitiv::ema::rdm::MMT_MARKET_PRICE)
					processMpcOnStatusMsg(*statusMsg, queuedMessage.pConsumerThread);
			...
			{
				GenericMsg* genericMsg = static_cast<GenericMsg*>(queuedMessage.pMsg);

				if (genericMsg->getDomainType() == refinitiv::ema::rdm::MMT_MARKET_PRICE)
					processMpcOnGenericMsg(*genericMsg, queuedMessage.pConsumerThread);
				else
					processMboOnGenericMsg(*genericMsg, queuedMessage.pConsumerThread);

				genericMsg->clear();
				_genericMsgPool.push(genericMsg);
				break;
			}
			default:
			{
				EmaString text("Warning: Unexpected message type '");
				text += queuedMessage.pMsg->getDataType();
				text += "'.\n";
				AppUtil::log(text.c_str());
			}
			}
		}
	}
