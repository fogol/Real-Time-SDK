...
void encodeETAUpdateMsg(RsslBuffer& msgBuf)
{
	...
	if ((retval = rsslEncodeMsg(&encIter, (RsslMsg*)&update)) < RSSL_RET_SUCCESS)
	{
		EXPECT_FALSE(true) << "rsslEncodeMsg() failed with return code: " << retval << std::endl;
	}
}

void encodeEMAUpdateMsg(UpdateMsg& updateMsg)
{
	updateMsg.streamId(15);
	updateMsg.seqNum(22);

	updateMsg.name("ABCDEF");
	updateMsg.nameType(INSTRUMENT_NAME_RIC);
	updateMsg.serviceId(5); // either Service ID or NAME can be specified
	updateMsg.filter(12);
	updateMsg.id(21);
...
TEST(UpdateMsgTests, testUpdateMsgDecode)
{
	...
	try
	{
		std::array<char, 4028> msgBufData;

		RsslBuffer msgBuf;
		msgBuf.data = msgBufData.data();
		msgBuf.length = static_cast<UInt32>(msgBufData.size());

		encodeETAUpdateMsg(msgBuf);

		UpdateMsg respMsg;

		StaticDecoder::setRsslData(&respMsg, &msgBuf, RSSL_DT_MSG, RSSL_RWF_MAJOR_VERSION, RSSL_RWF_MINOR_VERSION, &dictionary);
...
TEST(UpdateMsgTests, testUpdateMsgWithJson)
{
	...
	try
	{
		...
		rsslUpdateMsg.msgBase.encDataBody = rsslBuf;
		rsslUpdateMsg.msgBase.containerType = RSSL_DT_JSON;

		UpdateMsg updateMsg;

		StaticDecoder::setRsslData(&updateMsg, (RsslMsg*)&rsslUpdateMsg, RSSL_RWF_MAJOR_VERSION, RSSL_RWF_MINOR_VERSION, 0);

		EXPECT_EQ(updateMsg.getPayload().getDataType(), DataType::JsonEnum) << "UpdateMsg::getPayload().getDataType() == DataType::JsonEnum";

		EmaBuffer compareTo(jsonValue.data, jsonValue.length);
		EXPECT_STREQ(updateMsg.getPayload().getJson().getBuffer(), compareTo) << "UpdateMsg::getPayload().getJson().getBuffer()";
	}
	catch (const OmmException&)
	{
		EXPECT_FALSE(true) << "UpdateMsg Decode with Json payload - exception not expected";
	}
}
...
TEST(UpdateMsgTests, testUpdateMsgEncodeDecode)
{
	...
	try
	{
		...
		EXPECT_NE( -1, update.toString().find("ErrorCode=\"NoDictionary\"") ) << "UpdateMsg.toString() == toString() method can be used for just encoded object when the dictionary was previously loaded. Use toString(dictionary) for just encoded object.";
...
struct UpdateMsg_forClone
{
	...
	UpdateMsg_forClone() {
		// verify that the UpdateMsg class implements copy and move assignment operations
		// (they are required for cloning and storing in standard containers)
		static_assert(std::is_copy_constructible<UpdateMsg>::value,
			"UpdateMsg class must be copy constructible");
		static_assert(std::is_copy_assignable<UpdateMsg>::value,
			"UpdateMsg class must be copy assignable to enable efficient cloning");
		static_assert(std::is_move_assignable<UpdateMsg>::value,
			"UpdateMsg class must be move assignable");
		static_assert(std::is_nothrow_move_assignable<UpdateMsg>::value,
			"UpdateMsg class move assignment operator must not throw exceptions");
	...
	void encode_forClone(UpdateMsg& respMsg)
	{
		rsslClearUpdateMsg(&update);
		...
		update.msgBase.containerType = RSSL_DT_FIELD_LIST;
		update.msgBase.domainType = RSSL_DMT_MARKET_BY_PRICE;
		update.msgBase.streamId = 15;

		update.flags |= RSSL_UPMF_HAS_SEQ_NUM;
		update.seqNum = 22;

		update.flags |= RSSL_UPMF_DO_NOT_CACHE | RSSL_UPMF_DO_NOT_CONFLATE;

		update.flags |= RSSL_UPMF_HAS_POST_USER_INFO;
		update.postUserInfo.postUserAddr = 15;
		update.postUserInfo.postUserId = 30;
	...
	bool check_afterClone(const UpdateMsg& msg)
	{
		...
		*/
		EXPECT_EQ(msg.toString(), toString) << "Check equal toString()";

		EXPECT_EQ(msg.getDomainType(), RSSL_DMT_MARKET_BY_PRICE) << "UpdateMsg.domainType()";
		EXPECT_EQ(msg.getStreamId(), 15) << "UpdateMsg.streamId()";

		EXPECT_TRUE(msg.hasSeqNum()) << "UpdateMsg.hasSeqNum()";
		EXPECT_EQ(msg.getSeqNum(), 22) << "UpdateMsg.seqNum()";

		EXPECT_TRUE(msg.hasMsgKey()) << "UpdateMsg.hasMsgKey()";

		EXPECT_TRUE(msg.hasServiceId()) << "UpdateMsg.hasServiceId()";
		EXPECT_EQ(msg.getServiceId(), 2) << "UpdateMsg.serviceId()";

		EXPECT_TRUE(msg.hasId()) << "UpdateMsg.hasId()";
		EXPECT_EQ(msg.getId(), 4) << "UpdateMsg.id()";

		EXPECT_TRUE(msg.hasFilter()) << "UpdateMsg.hasFilter()";
		EXPECT_EQ(msg.getFilter(), 8) << "UpdateMsg.hasFilter()";

		// todo: service name needs special treatment, gets lost during translation of the
		//       original message to buffer (setData method)

		// EXPECT_TRUE(msg.hasServiceName()) << "UpdateMsg.hasServiceName()";
		// EXPECT_EQ(msg.getServiceName(), "DIRECT_FEED") << "UpdateMsg.serviceName()";

		EXPECT_TRUE(msg.hasNameType()) << "UpdateMsg.hasNameType()";
		EXPECT_EQ(msg.getNameType(), INSTRUMENT_NAME_RIC) << "UpdateMsg.nameType()";

		EXPECT_TRUE(msg.hasName()) << "UpdateMsg.hasName()";
		EXPECT_EQ(msg.getName(), "ABCDEF") << "UpdateMsg.name()";

		EXPECT_EQ(msg.getAttrib().getDataType(), DataType::FieldListEnum) << "UpdateMsg.attrib().dataType()";

		EXPECT_TRUE(msg.getDoNotCache()) << "UpdateMsg.doNotCache()";
		EXPECT_TRUE(msg.getDoNotConflate()) << "UpdateMsg.doNotconflate()";
		EXPECT_TRUE(!msg.getDoNotRipple()) << "UpdateMsg.solicited()";
		EXPECT_FALSE(msg.hasConflated()) << "UpdateMsg.hasConflated()";

		EXPECT_TRUE(msg.hasPublisherId()) << "UpdateMsg.hasPublisherId()";
		EXPECT_EQ(msg.getPublisherIdUserAddress(), 15) << "UpdateMsg.publisherIdUserAddress()";
		EXPECT_EQ(msg.getPublisherIdUserId(), 30) << "UpdateMsg.publisherIdUserId()";

		EXPECT_EQ(msg.getPayload().getDataType(), DataType::FieldListEnum) << "UpdateMsg.payload().dataType()";
		return true;
	}
...
TEST(UpdateMsgTests, testUpdateMsgClone_CopyConstruct)
{
	UpdateMsg_forClone encoder;

	const UpdateMsg respMsg = encoder.generateMessage();

	// Clone message
	UpdateMsg cloneUpdateMsg(respMsg);

	EXPECT_TRUE(encoder.check_afterClone(cloneUpdateMsg)) << "UpdateMsg Clone Success";
}

TEST(UpdateMsgTests, testUpdateMsgClone_CopyAssign)
{
	UpdateMsg_forClone encoder;
	const UpdateMsg respMsg = encoder.generateMessage();
	...
	cloneUpdateMsg = respMsg;

	EXPECT_TRUE(encoder.check_afterClone(cloneUpdateMsg)) << "UpdateMsg Clone Success";
}

TEST(UpdateMsgTests, testUpdateMsgClone_CopyAssignReserved)
{
	UpdateMsg_forClone encoder;
	const UpdateMsg _tmp = encoder.generateMessage();
	...
	for (const UInt32 reservation : { 0, 1, 3, 15, 1024 })
	{
		...
		StaticDecoder::setRsslData(&srcMsg, (RsslMsg*)&encoder.update, RSSL_RWF_MAJOR_VERSION, RSSL_RWF_MINOR_VERSION, &encoder.dictionary);

		UpdateMsg cloneUpdateMsg{reservation};

		// Clone message via copy assignment
		cloneUpdateMsg = srcMsg;

		EXPECT_TRUE(encoder.check_afterClone(cloneUpdateMsg)) << "UpdateMsg Clone Success";
	}
}

TEST(UpdateMsgTests, testUpdateMsgClone_MoveConstruct)
{
	UpdateMsg_forClone encoder;
	UpdateMsg msg = encoder.generateMessage();

	// Clone message via copy construct
	UpdateMsg cloneUpdateMsg{std::move(msg)};

	EXPECT_TRUE(encoder.check_afterClone(cloneUpdateMsg)) << "UpdateMsg Clone Success";
}


TEST(UpdateMsgTests, testUpdateMsgClone_MoveAssign)
{
	UpdateMsg_forClone encoder;
	UpdateMsg respMsg = encoder.generateMessage();

	UpdateMsg cloneUpdateMsg;

	// Clone message via move assign
	cloneUpdateMsg = std::move(respMsg);
