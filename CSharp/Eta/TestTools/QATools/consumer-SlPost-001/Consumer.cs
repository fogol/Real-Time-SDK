...
namespace LSEG.Eta.Example.Consumer
{
    public class Consumer : ResponseCallback
    {
        ...
        private bool shouldSymbolListPost = false;
        ...
        public void Init(string[] args)
        {
            ...
            shouldSymbolListPost = CommandLine.BoolValue("slpost");
        ...
        private void HandlePosting()
        {
            ...
            {
                postHandler.OffStreamId = loginHandler.LoginRefresh.StreamId;
                postHandler.OffStreamPostItemName.Data("OFFPOST");
            }
            if (postHandler.EnableSymbolListPost)
            {
                postHandler.SlStreamId = loginHandler.LoginRefresh.StreamId;
                postHandler.SlPostItemName.Data("_SYMBOL_LIST_POST");
            }

            TransportReturnCode ret = postHandler.HandlePosts(channelSession, out error);
            if (ret < TransportReturnCode.SUCCESS)
                Console.WriteLine("Error posting: " + error.Text);
        }
        ...
        private static void AddCommandLineArgs()
        {
            ...
            CommandLine.AddOption("slpost", "Specifies that the application should attempt to send Symbol List post messages on the login stream (i.e., off-stream).");
