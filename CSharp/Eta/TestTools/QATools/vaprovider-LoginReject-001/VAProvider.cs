...
namespace LSEG.Eta.ValueAdd.Provider
{
    public class VAProvider : IRDMLoginMsgCallback, IDirectoryMsgCallback, IDictionaryMsgCallback, IReactorChannelEventCallback, IDefaultMsgCallback
    {
        ...
        //APIQA
        private int n_ItemsToSend;
        private const int defaultNItemsToSend = 0;
        ...
        private void Init(string[] args)
        {
            ...
            //APIQA
            if (m_ProviderCmdLineParser.NItemsToSend != 0)
            {
                n_ItemsToSend = m_ProviderCmdLineParser.NItemsToSend;
            }
            else
            {
                n_ItemsToSend = defaultNItemsToSend;
            }
            ...
            if (m_ProviderCmdLineParser.ConnectionType == ConnectionType.ENCRYPTED)
            {
                ...
                Console.WriteLine($"Tls: {m_ProviderCmdLineParser.Protocol}");
            }
            ...
            if (m_ProviderCmdLineParser.ConnectionType == ConnectionType.ENCRYPTED)
            {
                ...
                m_BindOptions.BindEncryptionOpts.EncryptionProtocolFlags = m_ProviderCmdLineParser.Protocol;
            }
	    ...
	    //APIQA
            m_ItemHandler.n_ItemsToSend = n_ItemsToSend;
        }
                ...
                if (m_CurrentSocketList.Count == 0)
                {
                    ...
                    foreach (ReactorChannel reactorChnl in m_ReactorChannelList)
                    {
                        if (reactorChnl != null && reactorChnl.State == ReactorChannelState.READY)
                        {
                            ...
                            if (ret != ReactorReturnCode.SUCCESS)
                            {
				//APIQA Added code to send failure after sending out a certain number of updates across all items
				if (ret == ReactorReturnCode.PARAMETER_OUT_OF_RANGE)
				{
				    ret = m_LoginHandler.SendCloseStatus(reactorChnl, out m_ErrorInfo);
				}
				else
				{
					if (reactorChnl.State != ReactorChannelState.CLOSED &&
