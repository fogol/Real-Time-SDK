...
namespace LSEG.Eta.ValueAdd.Provider
{
    internal class ProviderCmdLineParser : ICommandLineParser
    {
        ...
        public EncryptionProtocolFlags Protocol { get; set; }
        public ConnectionType ConnectionType { get; set; } = ConnectionType.SOCKET;

        //APIQA
        public int NItemsToSend { get; set; } = -1;
		...
		public bool ParseArgs(string[] args)
		{
			try
			{
				...
				while (argsCount < args.Length)
				{
                    ...
                    {
						var protocolValue = args[++argsCount];
						if(EncryptionProtocolFlagsExtension.TryParse(protocolValue, out var protocol))
						{
							Protocol = protocol;
                        }
						else
						{
                            Console.WriteLine("\nUnrecognized tls version...\n");
                            return false;
                        }
                    }
                    //APIQA
                    else if ("-ni".Equals(args[argsCount]))
                    {
                        NItemsToSend = int.Parse(args[++argsCount]);
                        ++argsCount;
                    }
		...
		public void PrintUsage()
		{
			Console.WriteLine("Usage: Provider or\nProvider [-p <port number>] [-i <interface name>] [-s <service name>] [-id <service ID>] [-runtime <seconds>]" +
                               ...
                               //APIQA
                               "\n -ni number of items to send before sending login closed suspect and cutting the channel");
		}
