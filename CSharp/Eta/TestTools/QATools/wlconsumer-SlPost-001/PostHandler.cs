...
public class PostHandler
{
    ...
    private List<SymbolListItem> m_SymbolListItemList = new List<SymbolListItem>();
    ...
    public Eta.Codec.Buffer OffStreamPostItemName { get; } = new();
    ...
    public Eta.Codec.Buffer SLPostItemName { get; } = new();
    ...
    internal CodecReturnCode HandlePosts(ReactorChannel chnl, out ReactorErrorInfo? errorInfo)
    {
        ...
        if (currentTime >= m_NextPostTime)
        {
            ...
            {
                CodecReturnCode ret = SendSymbolListPostMsg(chnl, out errorInfo);
                if (ret != CodecReturnCode.SUCCESS)
                {
                    return ret;
                }
    ...
    private CodecReturnCode SendOnstreamPostMsg(ReactorChannel chnl, bool postWithMsg, out ReactorErrorInfo? errorInfo)
    {
        ...
        if (postWithMsg)
        {
            if ((ret = EncodePostWithMsg(chnl, (int)DomainType.MARKET_PRICE, StreamId, PostItemName, ServiceId, out errorInfo)) != CodecReturnCode.SUCCESS)
            {
                return ret;
            }
        }
        ...
        {
            if ((ret = EncodePostWithData(chnl, StreamId, (int)DomainType.MARKET_PRICE, out errorInfo)) != CodecReturnCode.SUCCESS)
            {
                return ret;
            }
        }
    ...
    private CodecReturnCode SendOffstreamPostMsg(ReactorChannel chnl, bool postWithMsg, out ReactorErrorInfo? errorInfo)
    {
        ...
        if (postWithMsg)
        {
            if ((ret = EncodePostWithMsg(chnl, (int)DomainType.MARKET_PRICE, OffStreamId, OffStreamPostItemName, ServiceId, out errorInfo)) != CodecReturnCode.SUCCESS)
            {
                return ret;
            }
        }
        ...
        {
            if ((ret = EncodePostWithData(chnl, OffStreamId, (int)DomainType.MARKET_PRICE, out errorInfo)) != CodecReturnCode.SUCCESS)
            {
                return ret;
            }
        }
     ...
     * periodically over a time increment by the handlePosts method
     */
    private CodecReturnCode SendSymbolListPostMsg(ReactorChannel chnl, out ReactorErrorInfo? errorInfo)
    {
        errorInfo = null;
        if (SlStreamId == 0)
        {
        ...
        CodecReturnCode ret = EncodePostWithMsg(chnl, (int)DomainType.SYMBOL_LIST, SlStreamId, SLPostItemName, ServiceId, out errorInfo);
        if (ret != CodecReturnCode.SUCCESS)
        {
            return ret;
        }

        ReactorReturnCode reactorRet = chnl.Submit(m_PostMsg, m_SubmitOptions, out errorInfo);
        if (reactorRet != ReactorReturnCode.SUCCESS) 
            return CodecReturnCode.FAILURE;
        ...
        out ReactorErrorInfo? errorInfo)
    {
        ...
        else if (domainType == (int)DomainType.SYMBOL_LIST)
        {
            if (m_SlPostRefresh)
            {
                ...
                ret = EncodeSymbolListMsg((Msg)slRefreshMsg, m_PostMsgEncIter, SymbolListItems.SYMBOL_LIST_REFRESH, out errorInfo);
                if (ret != CodecReturnCode.SUCCESS)
                {
            ...
            else
            {
                ...
                ret = EncodeSymbolListMsg((Msg)slUpdateMsg, m_PostMsgEncIter, responseType, out errorInfo);
                if (ret != CodecReturnCode.SUCCESS)
                {
                    Console.WriteLine("EncodeSymbolListMsg() failed:  " + ret.GetAsString());
                    return CodecReturnCode.FAILURE;
    ...
    private CodecReturnCode EncodeSymbolListMsg(Msg msg, EncodeIterator encodeIter, int responseType, out ReactorErrorInfo errorInfo)
    {
        ...
        switch (responseType)
        {
            ...
            case SymbolListItems.SYMBOL_LIST_REFRESH:
                slBuffer.Data(m_SymbolListItemList[i].ItemName.Data());
        ...
        {
            for (i = 1; i < m_SymbolListItemList.Count; i++)
            {
                if (m_SymbolListItemList[i].IsInUse)
                {
                    slBuffer.Data(m_SymbolListItemList[i].ItemName.Data());
    ...
    /// </remarks>
    private CodecReturnCode EncodePostWithData(ReactorChannel chnl, 
        int streamId,
        int domain, 
        out ReactorErrorInfo? errorInfo)
    {
