...
/// <li>-clientId Specifies the client Id for V2 authentication OR specifies a unique ID, also known as AppKey, generated by an AppGenerator, for V1 authentication used when connecting to Real-Time Optimized.
...
public class WatchlistConsumer : IConsumerCallback, IReactorServiceEndpointEventCallback, IReactorOAuthCredentialEventCallback
{
    ...
    private void HandlePosting()
    {
        foreach (var chnlInfo in m_ChnlInfoList)
        {
            ...
            {
                WatchlistConsumerConfig.ItemInfo? postingItem = GetPostingItem((int)DomainType.MARKET_PRICE);
                if (postingItem == null) return;
            ...
            {
                chnlInfo.PostHandler.SlStreamId = chnlInfo.LoginRefresh.StreamId;
                chnlInfo.PostHandler.SLPostItemName.Data(m_WatchlistConsumerConfig.SlPostName != null ? m_WatchlistConsumerConfig.SlPostName : "_SYMBOL_LIST_POST");
            }
            if (chnlInfo.PostHandler.EnableOffStreamPost)
            {
                chnlInfo.PostHandler.OffStreamId = chnlInfo.LoginRefresh.StreamId;
                chnlInfo.PostHandler.OffStreamPostItemName.Data("OFFPOST");
                
            }

            CodecReturnCode ret = chnlInfo.PostHandler.HandlePosts(chnlInfo.ReactorChannel, out var errorInfo);
            if (ret < CodecReturnCode.SUCCESS)
                Console.WriteLine("Error posting: " + errorInfo?.Error.Text);
        }
    }

    private WatchlistConsumerConfig.ItemInfo? GetPostingItem(int domain)
    {
        WatchlistConsumerConfig.ItemInfo? postingItem = null;

        for (int i = 0; i < m_WatchlistConsumerConfig.ItemCount; i++)
        {
            if (m_WatchlistConsumerConfig.ItemList[i].Domain == domain)
            {
                ...
                {
                    Console.WriteLine("No currently available streams to post to. Will retry shortly.");
                    return null;
                }
                break;
            }
    ...
    private void InitChannelInfo(ChannelInfo chnlInfo)
    {
        ...
        chnlInfo.ShouldSymbolListPost = m_WatchlistConsumerConfig.EnableSymbolListPost;

        if (chnlInfo.ShouldOnStreamPost)
        {
            if (!IsItemFoundInItemList(chnlInfo, (int)DomainType.MARKET_PRICE))
            {
    ...
    private static bool IsItemFoundInItemList(ChannelInfo chnlInfo, int domain)
    {
