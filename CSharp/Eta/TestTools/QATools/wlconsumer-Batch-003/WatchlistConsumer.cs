...
public class WatchlistConsumer : IConsumerCallback, IReactorServiceEndpointEventCallback, IReactorOAuthCredentialEventCallback
{
    ...
    private CodecReturnCode RequestItems(ReactorChannel channel)
    {
        ...
        for (int itemListIndex = 0; itemListIndex < m_WatchlistConsumerConfig.ItemList.Count; ++itemListIndex)
        {
            ...
            *  END APIQA */
            ...
            {
                //Console.WriteLine("batchListIndex is" + batchListIndex);
                        ...
                        (!usedDomains.Contains(domainType)))
                {
                    ...
                    //Console.WriteLine ("---- APIQA:  addingItem " + m_WatchlistConsumerConfig.ItemList[batchListIndex].Name);
                }
            }
    ...
    int RequestMPItem(ReactorChannel channel, String name, bool isSnap)
    {
        ...
        m_ItemRequest.DomainType = (int)DomainType.MARKET_PRICE;
        m_ItemRequest.StreamId = (m_WatchlistConsumerConfig.ItemList[0].StreamId + m_WatchlistConsumerConfig.ItemList.Count + 2);
        m_ItemRequest.StreamId = 50;
        Console.WriteLine("---- APIQA: Requesting TRI again on user streamid  " + m_ItemRequest.StreamId);
            ...
            case ReactorChannelEventType.CHANNEL_OPENED:
                {
                        ...
                        || m_ItemDecoder.FieldDictionaryLoadedFromFile && m_ItemDecoder.EnumTypeDictionaryLoadedFromFile)
                    {
                        ...
                        RequestMPItem(reactorEvent.ReactorChannel!, "TRI", false);
                        //END APIQA
                    }
                    else
    ...
    public ReactorCallbackReturnCode DefaultMsgCallback(ReactorMsgEvent reactorEvent)
    {
        ...
        switch (msg.MsgClass)
        {
            case MsgClasses.REFRESH:
                ...
                //APIQA
                Console.WriteLine("\n\n------APIQA: Got Item Refresh, UserSpec on event's steamInfo is [" + reactorEvent.StreamInfo.UserSpec + "] \n\n");
                //END APIQA
                break;
            ...
            case MsgClasses.UPDATE:
                ...
                //APIQA
                Console.WriteLine("\n\n--- APIQA: Got Item Update, UserSpec on event's steamInfo is [" + reactorEvent.StreamInfo.UserSpec + "] \n\n");
                //END APIQA
            ...
            case MsgClasses.STATUS:
                ...
                //APIQA
                Console.WriteLine("---APIQA: Got Item Status, UserSpec on event's steamInfo is [" + reactorEvent.StreamInfo.UserSpec + "]");
                //END APIQA
    ...
    public ReactorCallbackReturnCode RdmDirectoryMsgCallback(RDMDirectoryMsgEvent reactorEvent)
    {
        ...
        switch (msgType)
        {
            case DirectoryMsgType.REFRESH:
                ...
                if (reactorEvent.ReactorChannel is not null && m_FieldDictionaryLoaded && m_EnumDictionaryLoaded
                         || m_ItemDecoder.FieldDictionaryLoadedFromFile && m_ItemDecoder.EnumTypeDictionaryLoadedFromFile)
                    RequestItems(reactorEvent.ReactorChannel!);
				//  END APIQA
