...
public class PostHandler
{
    ...
    public int NumMultipartMsgs { get; private set; } = 9;
    public int MultipartCount { get; private set; }
    public bool PostMultipart { get; set; }
    ...
    internal CodecReturnCode HandlePosts(ReactorChannel chnl, out ReactorErrorInfo? errorInfo)
    {
        ...
        if (currentTime >= m_NextPostTime)
        {
            if (EnableOnStreamPost)
            {
                ...
                if (PostMultipart)
                {
                    CodecReturnCode ret = SendOnstreamMultipartPostMsg(chnl, m_PostWithMsg, out errorInfo);
            ...
            if (EnableOffStreamPost)
            {
                ...
                if (PostMultipart)
                {
                    CodecReturnCode ret = SendOffstreamMultipartPostMsg(chnl, m_PostWithMsg, out errorInfo);
                    if (ret != CodecReturnCode.SUCCESS)
                    {
                        return ret;
                    }
    ...
    // APIQA
    private CodecReturnCode SendOnstreamMultipartPostMsg(ReactorChannel chnl, bool postWithMsg, out ReactorErrorInfo? errorInfo)
    {
        if (StreamId == 0)
        {
            // no items available to post on
        ...
        CodecReturnCode ret = 0;
        if ((ret = EncodeMultipartPostWithMsg(chnl, out errorInfo)) != CodecReturnCode.SUCCESS)
        {
            errorInfo = null;
            return ret;
        }
    ...
    private CodecReturnCode SendOffstreamMultipartPostMsg(ReactorChannel chnl, bool postWithMsg, out ReactorErrorInfo? errorInfo)
    {
        CodecReturnCode ret = 0;
        if ((ret = EncodeMultipartPostWithMsg(chnl, out errorInfo)) != CodecReturnCode.SUCCESS)
        {
        ...
        // send post message
        if (chnl.Submit(m_PostMsg, m_SubmitOptions, out errorInfo) != ReactorReturnCode.SUCCESS)
            return CodecReturnCode.FAILURE;
    ...
    private CodecReturnCode EncodeMultipartPostWithMsg(ReactorChannel chnl, out ReactorErrorInfo? errorInfo)
    {
        // First encode message for payload
        m_PostMsg.Clear();
        ...
        if ((MultipartCount < NumMultipartMsgs) && m_PostRefresh)
        {
            m_PostMsg.ApplyHasPartNum();
            m_PostMsg.PartNum = (MultipartCount);
        }
        if ((MultipartCount == (NumMultipartMsgs - 1)) && m_PostRefresh)
        {
            m_PostMsg.ApplyPostComplete();
        }
        // populate post user info        
        ...
        else
        {
            try
            {
                m_PostMsg.PostUserInfo.UserAddrFromString(Dns.GetHostAddresses(Dns.GetHostName())
                           .Where(ip => ip.AddressFamily == AddressFamily.InterNetwork)
                           .Select(a => a.ToString())
                           .FirstOrDefault());
            }
        ...
        {
            errorInfo = new();
            errorInfo.Error.Text = "EncodeIter.SetBufferAndRWFVersion() failed:  <" + CodecReturnCodeExtensions.GetAsString(ret);
            return CodecReturnCode.FAILURE;
        }
        ...
        if (m_PostRefresh)
        {
            ...
            {
                errorInfo = new();
                errorInfo.Error.Text = "encodeMarketPriceRefresh() failed:  <" + CodecReturnCodeExtensions.GetAsString(ret);
                return CodecReturnCode.FAILURE;
            }
            // APIQA: 
            if ((MultipartCount == (NumMultipartMsgs - 1)) && m_PostRefresh)
        ...
        else
        {
            ...
            {
                errorInfo = new ();
                errorInfo.Error.Text = "encodeMarketPriceUpdate() failed:  <" + CodecReturnCodeExtensions.GetAsString(ret);
                return CodecReturnCode.FAILURE;
            }
        }
