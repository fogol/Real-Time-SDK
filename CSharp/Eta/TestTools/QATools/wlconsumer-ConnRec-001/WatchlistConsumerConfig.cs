...
AddCommandLineArgs();
        try
        {
            ...
            if (NumConnections > 1)
            {
                if (NumConnections == 2)
                {
                    ConnectionArg connectionArg2 = new ConnectionArg();
                    connectionArg2.ConnectionType = ConnectionType.SOCKET;
                    connectionArg2.Service = ServiceName!;
                    connectionArg2.Hostname = Hostname2!;
                    connectionArg2.Port = Port2!;
                    connectionArg2.InterfaceName = Value("if");
                    connectionArg2.ItemList = itemList;
                    ConnectionList.Add(connectionArg2);
                }
                else if (NumConnections == 3)
                {
                    ...
                    ConnectionArg connectionArg3 = new ConnectionArg();
                    connectionArg3.ConnectionType = ConnectionType.SOCKET;
                    connectionArg3.Service = ServiceName!;
                    connectionArg3.Hostname = Hostname3!;
                    connectionArg3.Port = Port3!;
                    connectionArg3.InterfaceName = Value("if");
                    connectionArg3.ItemList = itemList;
                    ConnectionList.Add(connectionArg3);
                }
                else
                {
                    Console.Error.WriteLine("Error loading command line arguments:\t");
                    Console.WriteLine("QA NumConnections = " + NumConnections! + " is NOT valid.");
    ...
    //API QA
    public int ReconnectAttemptLimit => IntValue("attemptLimit");
    public int ReconnectMinDelay => IntValue("minDelay");
    public int ReconnectMaxDelay => IntValue("maxDelay");
    public int NumConnections => IntValue("numConnections");
    public string? Hostname2 => Value("h2");
    public string? Port2 => Value("p2");
    public string? Hostname3 => Value("h3");
    public string? Port3 => Value("p3");
    //End API QA
    ...
    public void AddCommandLineArgs()
    {
        ...
        AddOption("maxDelay", DEFAULT_REC_AT_MAX_DELAY, "ReconnectMaxDelay in milliseconds");
        AddOption("numConnections", DEFAULT_NUM_CONNECTIONS, "Specify number of connections (only limit to 3");
        AddOption("h2", "Server host2 name, use only when specify numConnections 2");
        AddOption("p2", "Server port2 number, use only when specify numConnections 2");
        AddOption("h3", "Server host3 name, use only when specify numConnections 3");
        AddOption("p3", "Server port3 number, use only when specify numConnections 3");
        //End API QA
    }
}
