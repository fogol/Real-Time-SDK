...
namespace LSEG.Eta.ValueAdd.Consumer
{
    ...
    internal class ConsumerCmdLineParser : ICommandLineParser
    {
        ...
        /* API QA */
        internal string? ClientId1 { get; private set; }

        internal string? ClientSecret1 { get; private set; }

        internal string? JwkFile1 { get; private set; }

        internal string? Audience1 { get; private set; }

        internal string? ClientId2 { get; private set; }

        internal string? ClientSecret2 { get; private set; }

        internal string? JwkFile2 { get; private set; }
        ...
        public bool ParseArgs(string[] args)
        {
            ...
            while (argsCount < args.Length)
            {
                ...
                else if ("-restProxyHostname".Equals(args[argsCount]))
                {
        ...
        public void PrintUsage()
        {
            Console.WriteLine("Usage: Consumer or\nConsumer [-c <hostname>:<port>[,...] <service name> <domain>:<item name>,...] [-bc <hostname>:<port>] [-uname <LoginUsername>] [-view] [-post] [-offpost] [-snapshot] [-runtime <seconds>]" +
                               ...
                               /* API QA */
                               "\n -clientId1 specifies a unique ID for application making the request to RDP token service" +
                               "\n -clientSecret1 specifies the associated secret with the client ID" +
                               "\n -jwkFile1 specifies a file containing the JWK encoded private key for V2 JWT logins." +
                               "\n -audience1 audience claim for v2 JWT logins." +
                               "\n -clientId2 specifies a unique ID for application making the request to RDP token service" +
                               "\n -clientSecret2 specifies the associated secret with the client ID" +
                               "\n -jwkFile2 specifies a file containing the JWK encoded private key for V2 JWT logins." +
