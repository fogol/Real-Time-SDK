...
namespace LSEG.Eta.ValueAdd.Consumer
{
    ...
    public class VAConsumer : IConsumerCallback, IReactorAuthTokenEventCallback, IReactorOAuthCredentialEventCallback
    {
        ...
        /// default EncryptionProtocolFlags
        private const EncryptionProtocolFlags DEFAULT_TLS = EncryptionProtocolFlags.ENC_NONE;
        ...
        private void Init(string[] args)
        {
            ...
            if (!string.IsNullOrEmpty(m_ConsumerCmdLineParser.RestProxyHostname))
            {
                var rPOpt = m_ReactorOptions.RestProxyOptions;
                rPOpt.ProxyHostName = m_ConsumerCmdLineParser.RestProxyHostname;
                rPOpt.ProxyPort = m_ConsumerCmdLineParser.RestProxyPort;
                rPOpt.ProxyUserName = m_ConsumerCmdLineParser.RestProxyUsername;
                rPOpt.ProxyPassword = m_ConsumerCmdLineParser.RestProxyPasswd;
            }
            ...
            int i = 0;
                foreach (ConnectionArg connectionArg in m_ConsumerCmdLineParser.ConnectionList)
                {
                    ...
                    if (i==0)
                    {
                        if (!string.IsNullOrEmpty(m_ConsumerCmdLineParser.ClientId1))
                        {
                            oAuthCredential.ClientId.Data(m_ConsumerCmdLineParser.ClientId1);

                            if (!string.IsNullOrEmpty(m_ConsumerCmdLineParser.ClientSecret1))
                            {
                                oAuthCredential.ClientSecret.Data(m_ConsumerCmdLineParser.ClientSecret1);
                        ...
                        if (!string.IsNullOrEmpty(m_ConsumerCmdLineParser.JwkFile1))
                        {
                            oAuthCredential.ClientJwk.Data(m_ConsumerCmdLineParser.JwkFile1);
                        }

                        if (!string.IsNullOrEmpty(m_ConsumerCmdLineParser.Audience1))
                        {
                            oAuthCredential.Audience.Data(m_ConsumerCmdLineParser.Audience1);
                        }
                    }
                    else
                    {
                        if (!string.IsNullOrEmpty(m_ConsumerCmdLineParser.ClientId2))
                        {
                            oAuthCredential.ClientId.Data(m_ConsumerCmdLineParser.ClientId2);

                            if (!string.IsNullOrEmpty(m_ConsumerCmdLineParser.ClientSecret2))
                            {
                                oAuthCredential.ClientSecret.Data(m_ConsumerCmdLineParser.ClientSecret2);
                        ...
                        if (!string.IsNullOrEmpty(m_ConsumerCmdLineParser.JwkFile2))
                        {
                            oAuthCredential.ClientJwk.Data(m_ConsumerCmdLineParser.JwkFile2);
                        }

                        if (!string.IsNullOrEmpty(m_ConsumerCmdLineParser.Audience2))
                        {
                            oAuthCredential.Audience.Data(m_ConsumerCmdLineParser.Audience2);
                        }
                    }
                    /* End API QA */
                    ...
                    {
                        Console.WriteLine("QA reactor.connect SUCCESS for index:" + i);
                    }
                    /* End API QA */
