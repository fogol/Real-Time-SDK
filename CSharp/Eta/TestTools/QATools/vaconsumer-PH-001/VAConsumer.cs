...
namespace LSEG.Eta.ValueAdd.Consumer
{
    ...
    public class VAConsumer : IConsumerCallback, IReactorAuthTokenEventCallback, IReactorOAuthCredentialEventCallback
    {
        ...
        public ReactorCallbackReturnCode ReactorChannelEventCallback(ReactorChannelEvent evt)
        {
            ...
            switch (evt.EventType)
            {
                ...
                case ReactorChannelEventType.CHANNEL_DOWN_RECONNECTING:
                    {
                        if (!evt.ReactorChannel?.Socket?.SafeHandle.IsClosed ?? false)
                            //APIQA
                            Console.WriteLine($"\nConnection down reconnecting: " + FormatReactorChannel(evt.ReactorChannel!));
                        //END APIQA
                        else
        ...
        private void InitChannelInfo(ChannelInfo chnlInfo)
        {
            ...
            */
            chnlInfo.ConnectOptions.SetReconnectAttempLimit(m_ConsumerCmdLineParser.ReconnectAttemptLimit); // attempt to recover forever
            chnlInfo.ConnectOptions.SetReconnectMinDelay(m_ConsumerCmdLineParser.ReconnectMinDelay); // 1 second minimum
            chnlInfo.ConnectOptions.SetReconnectMaxDelay(m_ConsumerCmdLineParser.ReconnectMaxDelay); // 60 second maximum
            //END API QA
