...
namespace LSEG.Eta.ValueAdd.Consumer
{
    ...
    internal class ConsumerCmdLineParser : ICommandLineParser
    {
        ...
        //API QA
        public int ReconnectAttemptLimit { get; private set; } = -1;
        public int ReconnectMinDelay { get; private set; } = 500;
        public int ReconnectMaxDelay { get; private set; } = 6000;
        ...
        public bool ParseArgs(string[] args)
        {
            ...
            while (argsCount < args.Length)
            {
                ...
                {
                    if (int.TryParse(args[++argsCount], out var reconnectAttemptLimit))
                    {
                        ReconnectAttemptLimit = reconnectAttemptLimit;
                    }
                    else
                    {
                        Console.WriteLine($"Error: failed to parse ReconnectAttemptLimit '{args[argsCount]}'");
                        return false;
                    }
                    ++argsCount;
                }
                ...
                {
                    if (int.TryParse(args[++argsCount], out var reconnectMinDelay))
                    {
                        ReconnectMinDelay = reconnectMinDelay;
                    }
                    else
                    {
                        Console.WriteLine($"Error: failed to parse ReconnectMinDelay '{args[argsCount]}'");
                        return false;
                    }
                    ++argsCount;
                }
                ...
                {
                    if (int.TryParse(args[++argsCount], out var reconnectMaxDelay))
                    {
                        ReconnectMaxDelay = reconnectMaxDelay;
                    }
                    else
                    {
                        Console.WriteLine($"Error: failed to parse ReconnectMaxDelay '{args[argsCount]}'");
                        return false;
                    }
                    ++argsCount;
                }
