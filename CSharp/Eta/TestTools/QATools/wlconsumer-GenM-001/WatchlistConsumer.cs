...
public class WatchlistConsumer : IConsumerCallback, IReactorServiceEndpointEventCallback, IReactorOAuthCredentialEventCallback
{
    ...
    // APIQA
    private bool m_NewSrcDirRequested = false;
    private int m_UpdatesReceived = 0;
    private bool m_ItemClosed = false;
    // END APIQA
    ...
    private CodecReturnCode CloseItem(Reactor.Reactor reactor, ReactorChannel channel)
    {
        ...
        int domainType = m_WatchlistConsumerConfig.ItemList[2].Domain;
        int streamId = m_WatchlistConsumerConfig.ItemList[2].StreamId;
        ...
        if (channel.Submit((Msg)m_CloseMsg, m_SubmitOptions, out _) != ReactorReturnCode.SUCCESS)
        {
    ...
    // APIQA
    private void RequestNewSourceDirectory(ChannelInfo chnlInfo)
    {
        if (m_NewSrcDirRequested)
        {
            return;
        }
    ...
    public ReactorCallbackReturnCode DefaultMsgCallback(ReactorMsgEvent reactorEvent)
    {
        ...
        switch (msg.MsgClass)
        {
            ...
            case MsgClasses.GENERIC:
                IGenericMsg genMsg = (IGenericMsg)msg;
                Console.WriteLine("Received GENERIC message on streamId " + msg.StreamId + " with msgkey: serviceid=" + msg.MsgKey.ServiceId + ", name of " + msg.MsgKey.Name.ToString()
                        + "\n\n");
                Console.WriteLine("\n-------------APIQA: Received GENERIC message on streamId " + msg.StreamId + " with msgKey serviceId of " + msg.MsgKey.ServiceId + " \n\n");
                m_ItemDecoder.DecodeDataBody(reactorEvent.ReactorChannel!, genMsg);
                SendGenericMessageLogin(reactorEvent.ReactorChannel!);
                SendGenericMessageDirectory(reactorEvent.ReactorChannel!);
                break;
            // END APIQA:
    ...
    {
        Msg msg = new();
        Eta.Codec.Buffer genericBuffer = new();
        genericBuffer.Data(new ByteBuffer(512));
        ...
        Eta.Codec.Buffer entryName = new();
        ...
        encodeIter.Clear();
        ret = encodeIter.SetBufferAndRWFVersion(genericBuffer, chnl.MajorVersion, chnl.MinorVersion);

        ret = map.EncodeInit(encodeIter, 100, 0);
        if (ret < CodecReturnCode.SUCCESS)
        {
            Console.WriteLine("fail to encode map");
        }
        ret = mEntry.EncodeInit(encodeIter, uintValue, 0);
        if (ret < CodecReturnCode.SUCCESS)
        {
            Console.WriteLine("fail to encode map Entry");
        }
        ret = elementList.EncodeInit(encodeIter, null, 0);
        if (ret < CodecReturnCode.SUCCESS)
        {
            Console.WriteLine("fail to encode elementList");
        }
        ret = elementEntry.Encode(encodeIter, uintValue);
        if (ret < CodecReturnCode.SUCCESS)
        {
            Console.WriteLine("fail to encode elemenEntry");
        }
        ret = elementList.EncodeComplete(encodeIter, true);
        if (ret < CodecReturnCode.SUCCESS)
        {
            Console.WriteLine("fail to encodeComplet");
        }
        ...
        if (ret < CodecReturnCode.SUCCESS)
        {
            Console.WriteLine("fail to  mapComplet");
        }
        genericMsg.EncodedDataBody = genericBuffer;
        ReactorSubmitOptions reactorSubmitOptions = new ();
        reactorSubmitOptions.Clear();
        if (chnl.Submit((Msg)genericMsg, reactorSubmitOptions, out var errorInfo) != ReactorReturnCode.SUCCESS)
        {
    ...
    {
        Msg msg = new();
        Eta.Codec.Buffer _genericBuffer = new();
        _genericBuffer.Data(new ByteBuffer(512));
        IGenericMsg genericMsg = msg;
        var ret = CodecReturnCode.SUCCESS;
        genericMsg.Clear();
        genericMsg.MsgClass = MsgClasses.GENERIC;
        ...
        if (chnl.Submit((Msg)genericMsg, reactorSubmitOptions, out var errorInfo) != ReactorReturnCode.SUCCESS)
        {
            Console.WriteLine("\n\n-------------APIQ: Error submitting generic message on LOGIN domain: " + errorInfo);
        }
        return;
    }
    // END APIQA
