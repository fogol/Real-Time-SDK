...
internal class ItemDecoder
{
    ...
    // APIQA
    /// <summary>
    /// Decodes a GenericMsg payload. GenericMsg contain ElementList with ElementEntry 
    /// </summary>
    private CodecReturnCode DecodeGenericMsgDataBody(ReactorChannel channel, IMsg msg)
    {
        ...
        if (msg.ContainerType != DataTypes.ELEMENT_LIST)
            return CodecReturnCode.SUCCESS; // nothing to extract
        m_DIter.SetBufferAndRWFVersion(msg.EncodedDataBody, channel.MajorVersion, channel.MinorVersion);
        CodecReturnCode ret = elementList.Decode(m_DIter, null);
        if (ret != CodecReturnCode.SUCCESS)
        {
            Console.WriteLine($"elementList.Decode() failed with return code: {ret}");
            return ret;
        }
        while ((ret = elementEntry.Decode(m_DIter)) != CodecReturnCode.END_OF_CONTAINER)
        {
            if (ret != CodecReturnCode.SUCCESS)
            {
                Console.WriteLine($"elementEntry.Decode() failed with return code: {ret}");
                return ret;
            }

            if (elementEntry.DataType == DataTypes.INT)
            {
                // Nothing to extract
                Console.WriteLine($"GenericMsg on StreamId : {msg.StreamId}  ElementEntryName:  {elementEntry.Name}");
                return CodecReturnCode.SUCCESS;
            }
