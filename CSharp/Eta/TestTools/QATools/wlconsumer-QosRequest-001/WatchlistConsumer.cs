...
using ItemInfo = LSEG.Eta.ValueAdd.WatchlistConsumer.WatchlistConsumerConfig.ItemInfo;
...
public class WatchlistConsumer : IConsumerCallback, IReactorServiceEndpointEventCallback, IReactorOAuthCredentialEventCallback
{
    ...
    CodecReturnCode RequestNewItem(ReactorChannel channel, bool existingItem, bool isPrivate, bool isSnap, bool isView, int viewId, bool isMsgKeyInUpdates, string name)
    {
    ...
    // APIQA: Reissue item
    private CodecReturnCode ReissueItem(ReactorChannel channel, int reissueType, int reissueViewId, int itemIndex)
    {
        CodecReturnCode ret = CodecReturnCode.SUCCESS;
        ...
        for (int itemListIndex = 0; itemListIndex < m_WatchlistConsumerConfig.ItemList.Count; ++itemListIndex)
        {
            ...
            */
            ...
            if (itemListIndex == 0)
            {
                ...
                m_ItemRequest.Qos.RateInfo(500);
                m_ItemRequest.Qos.Timeliness(3);
                m_ItemRequest.Qos.TimeInfo(500);             
            } else if (itemListIndex == 1)
            {
                m_ItemRequest.HasQos = true;
                m_ItemRequest.HasWorstQos = true;
                m_ItemRequest.Qos.IsDynamic = true;
                m_ItemRequest.Qos.Rate(1);
                m_ItemRequest.Qos.Timeliness(1);
                m_ItemRequest.WorstQos.Rate(3);
                m_ItemRequest.WorstQos.Timeliness(2);
                m_ItemRequest.WorstQos.RateInfo(500);
            }
            //End API QA QOS 0253
                        ...
                        for (int i = 0; i < m_AllEventCounters.Count; ++i)
                        {
                            if (m_AllEventCounters[i].Delay == m_E3Counter && m_AllEventCounters[i].EventType == 3)
                            {
                                ...
                                for (int index = startIdx; index < endIdx; ++index)
                                {
                                    ...
                                    RequestNewItem(reactorEvent.ReactorChannel!, false, isPrivate, isSnap, isView, vID, isMsgKeyInUpdates, nameOfItem);
                                }
                ...
                if (m_AllEventCounters.Count > 0)
                {
                    for (int i = 0; i < m_AllEventCounters.Count; ++i)
                    {
                        ...
                        if (m_AllEventCounters[i].Delay == m_E4Counter && m_AllEventCounters[i].EventType == 4)
                        {
                            ...
                            for (int index = startIdx; index < endIdx; ++index)
                            {
                                ReissueItem(reactorEvent.ReactorChannel!, reissueType, reissueVid, index);
                            }
                            m_AllEventCounters.RemoveAt(i);
                            break;
                        }
