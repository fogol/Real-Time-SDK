...
public class WatchlistConsumer : IConsumerCallback, IReactorServiceEndpointEventCallback, IReactorOAuthCredentialEventCallback
{
    ...
    private CodecReturnCode RequestItems(ReactorChannel channel)
    {
        ...
        //APIQA
        List<int> usedDomains = new List<int>();
        for (int itemListIndex = 0; itemListIndex < m_WatchlistConsumerConfig.ItemList.Count; ++itemListIndex)
        {
            ...
            //  APIQA
            for (int batchListIndex = itemListIndex; batchListIndex < m_WatchlistConsumerConfig.ItemList.Count; ++batchListIndex)
            {
                if (m_WatchlistConsumerConfig.ItemList[batchListIndex].Domain == m_WatchlistConsumerConfig.ItemList[itemListIndex].Domain &&
                        (!usedDomains.Contains(domainType)))
                {
                    m_ItemRequest.AddItem(m_WatchlistConsumerConfig.ItemList[batchListIndex].Name!);
                }
            }

            if (m_ItemRequest.GetItemNames().Count == 0)
            {
            ...
            //APIQA
            if (domainType == (int)DomainType.SYMBOL_LIST && m_WatchlistConsumerConfig.ItemList[itemListIndex].SymbolListData && m_ItemRequest.GetItemNames().Count == 1)
            //END APIQA
            {
            ...
            //APIQA
            else if (domainType == (int)DomainType.MARKET_PRICE && m_WatchlistConsumerConfig.EnableView && m_ItemRequest.GetItemNames().Count == 1)
            //END APIQA
            {
            ...
            //APIQA
            else if (m_ItemRequest.GetItemNames().Capacity > 1)
            {
                m_Payload = new(); // move it to the top to share 
                m_Payload.Data(new ByteBuffer(1024));
                ...
                WatchlistConsumerConfig.ItemInfo batchItemInfo = new WatchlistConsumerConfig.ItemInfo();
                batchItemInfo.Domain = domainType;
                batchItemInfo.Name = "";
                batchItemInfo.SymbolListData = false;
                batchItemInfo.IsPrivateStream = false;
                batchItemInfo.StreamId = m_ItemRequest.StreamId;
                batchItemInfo.IsBatchStream = true;

                for (int i = itemListIndex; i < m_WatchlistConsumerConfig.ItemList.Count; ++i)
                {
                    m_WatchlistConsumerConfig.ItemList[i].StreamId = m_WatchlistConsumerConfig.ItemList[i].StreamId + 1;
                }
                m_WatchlistConsumerConfig.ItemList[itemListIndex].State = batchItemInfo.State;
                m_WatchlistConsumerConfig.ItemList[itemListIndex].IsBatchStream = batchItemInfo.IsBatchStream;
                m_WatchlistConsumerConfig.ItemList[itemListIndex].IsPrivateStream = batchItemInfo.IsPrivateStream;
                m_WatchlistConsumerConfig.ItemList[itemListIndex].SymbolListData = batchItemInfo.SymbolListData;
                m_WatchlistConsumerConfig.ItemList[itemListIndex].Name = batchItemInfo.Name;
                m_WatchlistConsumerConfig.ItemList[itemListIndex].Domain = batchItemInfo.Domain;
                ...
                Console.WriteLine("\n---- APIQA: Sending batch request with " + m_ItemRequest.GetItemNames().Count + " items\n");
                if (domainType == (int)DomainType.MARKET_PRICE && m_WatchlistConsumerConfig.EnableView)
                {
                    m_ItemRequest.HasView = true;
                    m_ViewFieldList.Add(22);
                    m_ViewFieldList.Add(25);
                ...
                {
                    Console.WriteLine("ItemRequest.Encode() failed with code: ", ret);
                    return ret;
                }
                m_ItemRequest.RequestMsg.EncodedDataBody = m_Payload;
            }
            ...
            m_SubmitOptions.RequestMsgOptions.UserSpecObj = m_ItemRequest.StreamId + 1024;
            // APIQA:
            Console.WriteLine("\n\n----- APIQA: Submit: Setting UserSpec object to " + (m_ItemRequest.StreamId + 1024) + " for streamid " + m_ItemRequest.StreamId + "\n\n");
            // END APIQA
    ...
    //APIQA
    /* Requests the desired items.  */
    int RequestItemsAgain(ReactorChannel channel)
    {
        int itemListIndex = 0;
        List<int> usedDomains = new();
        for (; itemListIndex < m_WatchlistConsumerConfig.ItemList.Count; itemListIndex++)
        {
            int ret = (int)CodecReturnCode.SUCCESS;
            ...
            m_ItemRequest.DomainType = domainType;
            m_ItemRequest.StreamId = m_WatchlistConsumerConfig.ItemList[itemListIndex].StreamId + 50;

            for (int batchListIndex = itemListIndex; batchListIndex < m_WatchlistConsumerConfig.ItemList.Count; ++batchListIndex)
            {
                var batchInfo = m_WatchlistConsumerConfig.ItemList[batchListIndex];
                if (batchInfo.Domain == m_WatchlistConsumerConfig.ItemList[itemListIndex].Domain &&
                        (!usedDomains.Contains(domainType)) && batchInfo.Name is not null && batchInfo.Name != "")
                {
                    m_ItemRequest.AddItem(batchInfo.Name);
                }
            }
            ...
            if (domainType == (int)DomainType.SYMBOL_LIST && m_WatchlistConsumerConfig.ItemList[itemListIndex].SymbolListData && m_ItemRequest.GetItemNames().Count == 1)
            {
                ...
                ret = (int)m_ItemRequest.Encode(m_EncIter);

                if (ret < (int)CodecReturnCode.SUCCESS)
                {
                    Console.WriteLine("RequestItem.encode() failed. Error Id: ", ret);
                    return ret;
                }

                m_ItemRequest.RequestMsg.EncodedDataBody = m_Payload;
            }
            else if (domainType == (int)DomainType.MARKET_PRICE && m_WatchlistConsumerConfig.EnableView && m_ItemRequest.GetItemNames().Capacity == 1)
            {
                m_Payload = new(); // move it to the top to share 
                m_Payload.Data(new ByteBuffer(1024));
            ...
            else if (m_ItemRequest.GetItemNames().Capacity > 1)
            {
                ...
                Console.WriteLine("********* APIQA, requestItemsAgain: batch request streamID is " + batchItemInfo.StreamId + "\n\n\n");
            ...
            m_SubmitOptions.RequestMsgOptions.UserSpecObj = m_ItemRequest.StreamId + 1024;
            // APIQA:
            Console.WriteLine("\n\n------ APIQA: Submit AGAIN: Setting UserSpec object to " + (m_ItemRequest.StreamId + 1024) + " for streamid " + m_ItemRequest.StreamId + "\n\n");

            ret = (int)channel.Submit(m_ItemRequest.RequestMsg, m_SubmitOptions, out var errorInfo);
            if (ret < (int)CodecReturnCode.SUCCESS)
            {
    ...
    public ReactorCallbackReturnCode DefaultMsgCallback(ReactorMsgEvent reactorEvent)
    {
        ...
        switch (msg.MsgClass)
        {
            case MsgClasses.REFRESH:
                ...
                // APIQA:
                Console.WriteLine("\n\n------APIQA: Received Item Refresh, UserSpec on event's steamInfo is [" + reactorEvent.StreamInfo.UserSpec + "] \n\n");
                //END APIQA
                break;
            ...
            case MsgClasses.UPDATE:
                ...
                // APIQA:
                Console.WriteLine("\n\n------APIQA: Received Item Update, UserSpec on event's steamInfo is [" + reactorEvent.StreamInfo.UserSpec + "] \n\n");
                //END APIQA
                ...
                eventCounter++;
                if (reactorEvent.ReactorChannel is not null && eventCounter == 10)
                {
                    RequestItemsAgain(reactorEvent.ReactorChannel);
                }
                //END APIQA
                break;
            ...
            case MsgClasses.STATUS:
                ...
                // APIQA:
                Console.WriteLine("------APIQA: Received Item Status, UserSpec on event's steamInfo is [" + reactorEvent.StreamInfo.UserSpec + "]");
                //END APIQA
    ...
    public ReactorCallbackReturnCode RdmDirectoryMsgCallback(RDMDirectoryMsgEvent reactorEvent)
    {
        ...
        switch (msgType)
        {
            case DirectoryMsgType.REFRESH:
                ...
                {
                    RequestItems(reactorEvent.ReactorChannel!);
                    //RequestItemsAgain(reactorEvent.ReactorChannel.Reactor, reactorEvent.ReactorChannel);
					//RequestItemsAgain(m_Reactor, reactorEvent.ReactorChannel);
                 }
				//  END APIQA
