...
/// This message registers a user with the system.<br/>
/// After receiving a successful login response, applications can then begin consuming or
/// providing additional content.<br/>
/// An OMM provider can use the Login request information to authenticate users with DACS.<br/>
/// </remarks>
public sealed class LoginReq : Login
{
    ...
    /// </summary>
    /// <remarks>
    /// See <see cref="EmaRdm"/> for roles that can be set:<br/>
    ///    <see cref="EmaRdm.LOGIN_ROLE_CONS"/><br/>
    ///    <see cref="EmaRdm.LOGIN_ROLE_PROV"/><br/>
    /// </remarks>
    ...
    /// <summary>
    /// Returns DownloadConnectionConfig
    /// </summary>
    ...
    /// <summary>
    /// Returns ApplicationAuthorizationToken
    /// </summary>
    /// <returns>String representing ApplicationAuthorizationToken</returns>
    /// <exception cref="OmmInvalidUsageException">Thrown if <see cref="HasApplicationAuthorizationToken"/> returns false</exception>
    ...
    /// <exception cref="OmmInvalidUsageException">Thrown if <see cref="HasInstanceId"/> returns false</exception>
    ...
    /// <exception cref="OmmInvalidUsageException">Thrown if <see cref="HasPassword"/> returns false</exception>
    ...
    /// <summary>
    /// Returns AuthenticationExtended
    /// </summary>
    ...
    /// </summary>
    /// <returns>true if requesting PermissionExpressions, false if not requesting PermissionExpressions</returns>
    /// <exception cref="OmmInvalidUsageException">Thrown if <see cref="HasProvidePermissionExpressions"/> returns false</exception>
    ...
    /// <summary>
    /// Returns ProvidePermissionProfile
    /// </summary>
    ...
    /// <summary>
    /// Returns SupportProviderDictionaryDownload
    /// </summary>
    ...
    public override string ToString()
    {
        ...
        if (m_ApplicationAuthTokenSet)
        {
            m_ToString.AppendLine().Append(EmaRdm.ENAME_APPAUTH_TOKEN).Append(" : ").Append(m_ApplicationAuthToken);
        }

        if (m_DownloadConnectionConfigSet)
        {
            m_ToString.AppendLine().Append(EmaRdm.ENAME_DOWNLOAD_CON_CONFIG).Append(" : ").Append(m_DownloadConnectionConfig);
        }

        if (m_InstanceIdSet)
        {
            m_ToString.AppendLine().Append(EmaRdm.ENAME_INST_ID).Append(" : ").Append(m_InstanceId);
        }

        if (m_PasswordSet)
        {
            m_ToString.AppendLine().Append(EmaRdm.ENAME_PASSWORD).Append(" : ").Append(m_Password);
        }
        ...
        if (m_RoleSet)
        {
            m_ToString.AppendLine().Append(EmaRdm.ENAME_ROLE).Append(" : ").Append(m_Role);
        }
        ...
        {
            m_ToString.AppendLine().Append(EmaRdm.ENAME_SUPPORT_PROVIDER_DICTIONARY_DOWNLOAD).Append(" : ")
                .Append(m_SupportProviderDictionaryDownload);
        }

        if (m_PauseSet)
        {
            m_ToString.AppendLine().Append(EmaRdm.ENAME_PAUSE).Append(" : ").Append(m_Pause);
        }

        if (m_AuthenticationExtendedSet)
        {
            m_ToString.AppendLine().Append(EmaRdm.ENAME_AUTHN_EXTENDED).Append(" : ")
        ...
        if (m_NameSet)
        {
            if (m_NameTypeSet && m_NameType == EmaRdm.USER_AUTH_TOKEN)
            {
                m_ToString.AppendLine().Append(EmaRdm.ENAME_AUTHN_TOKEN).Append(" : ").Append(m_AuthenticationToken);
            }
            else
